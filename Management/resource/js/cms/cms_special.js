$(document).ready(function(){
    ...
    $("[nctype='btn_drop_special_image']").live("click",function(){
        ...
        $.getJSON("index.php?con=cms_special&fun=special_image_drop", { image_name: image_name }, function(result){
                if(result.status == "success") {
                    image_object.remove();
                } else {
    ...
    $("[nctype='btn_special_goods']").live('click', function(){
        ...
        $.getJSON("index.php?con=cms_special&fun=goods_info_by_url", { url: url}, function(data){
            if(data.result == "true") {
