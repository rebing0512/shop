...
<div id="pictureFolder" class="ncsc-picture-folder">
  <dl class="ncsc-album-intro">
    <dt class="albume-name"><a href="index.php?con=store_news&fun=album_pic_list&id=<?php echo $output['class_info']['aclass_id']?>"><?php echo $output['class_info']['aclass_name']?></a></dt>
    <dd class="album-covers">
      <?php if($output['class_info']['aclass_cover'] != ''){ ?>
      <img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_ARTICLE_ALBUM.DS.$_SESSION['store_id'].DS.str_replace('_small', '_tiny', $output['class_info']['aclass_cover']);?>">
      <?php }else{?>
    ...
    <div class="ad-nav">
      <div class="ad-thumbs">
        <ul class="ad-thumb-list">
          ...
          <li><a href="<?php echo sthumb($v);?>" value="<?php echo $v['apic_id'] ?>"> <img title="<?php echo $v['apic_name']?>" src="<?php echo sthumb($v, 60);?>" class="image0"/>
...
<script>
  $(function() {
	...
	$("#default_popup").click(function(){
		...
		<?php if (C('oss.open')) { ?>
		pic = pic.replace('@!product-240','');
		<?php } else { ?>
		pic = pic.replace('_240.'+img_type,'')+ '.' + img_type;		
        <?php } ?>
	...
	$("#tiny_popup").click(function(){
		...
		img_type = pic.substring(p);
		<?php if (C('oss.open')) { ?>
		pic = pic.replace('-240','-60');
		<?php } else { ?>
		pic = pic.replace('_240.','_60.');		
        <?php } ?>
	...
	$('.ad-back').live('click',function(){
		...
		$('.ad-thumb-list').load('index.php?con=store_news&fun=album_ad_ajax&id=<?php echo $_GET['id'];?>&class_id=<?php echo $_GET['class_id'];?>&curpage='+curpage);
	});
...
function ajax_change_imgmessage(url){
	...
	$.getJSON("<?php echo SHOP_SITE_URL; ?>/index.php?con=store_news&fun=ajax_change_imgmessage", {'url':url}, function(data){
