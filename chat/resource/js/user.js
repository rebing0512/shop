    ...
    $(function(){
		...
		if(user['u_id'] != '') {
			...
			var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>'+
									...
									'</div></div>';
			var ajaxurl = CHAT_SITE_URL+'/index.php?con=web_chat&fun=get_user_list&n=99&f_id='+user['u_id'];
			$.ajax({
				type: "GET",
				url: ajaxurl,
				dataType:"jsonp",
			    ...
			    success: function(u_list){
			  	for (var u_id in u_list){
					...
					setInterval( function () {
						$.get(CHAT_SITE_URL+'/index.php?con=web_chat&fun=get_session&key=member_id');
					}, time_max*60000);
	        ...
	        if ( act_op == 'goods_index' ) {//异步调用商品信息
				var ajaxurl = CHAT_SITE_URL+'/index.php?con=web_chat&fun=get_goods_info&goods_id='+chat_goods_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
	...
	function send_msg(){//发消息
		...
		$.ajax({
			type: "POST",
			url: CHAT_SITE_URL+'/index.php?con=web_chat&fun=send_msg',
			dataType:"jsonp",
			data: msg,
			async: false,
	...
	function get_msg(list){//接收消息
		...
		for (var k in list){
			...
			if ( typeof user_list[u_id]['avatar'] === "undefined" ) {//当没获得会员信息时调用一次
				var ajaxurl = CHAT_SITE_URL+'/index.php?con=web_chat&fun=get_info&t=member&u_id='+u_id;
				$.ajax({
					type: "GET",
					url: ajaxurl,
					dataType:"jsonp",
	...
	function get_chat_msg(t){
		var ajaxurl = CHAT_SITE_URL+'/index.php?con=web_chat&fun=get_chat_log&page=30&f_id='+user['u_id']+'&t_id='+chat_log['u_id']+'&t='+chat_log['time_from'];
	...
	function get_goods_info(msg){//显示商品图片和名称
	    if ( typeof msg['goods_id'] === "undefined" ) {//没有定义商品编号时调用检测一次
	        ...
	        var re = new RegExp(shop_site+"/index.php\\?con=goods&amp;fun=index&amp;goods_id=(\\d+)$","g");
    	    ...
    	    if ( goods_id > 0 ) {
    	        if ( typeof goods_list[goods_id] === "undefined" ) {
    				var ajaxurl = CHAT_SITE_URL+'/index.php?con=web_chat&fun=get_goods_info&goods_id='+goods_id;
    				$.ajax({
    					type: "GET",
    					url: ajaxurl,
    					dataType:"jsonp",
	...
	function get_goods_list(s_id){
		...
		var ajaxurl = CHAT_SITE_URL+'/index.php?con=web_chat&fun=get_goods_list&s_id='+s_id;
		$.ajax({
			type: "GET",
			url: ajaxurl,
			dataType:"jsonp",
	...
	function show_msg(u_id){//显示会员的消息
		...
		if(user_show != u_id) {
			...
			if (typeof user_info['s_name'] !== "undefined" && user_info['s_name'] !== "") {
			    ...
			    store_html = '<a target="_blank" href="'+SHOP_SITE_URL+'/index.php?con=show_store&fun=index&store_id='+
