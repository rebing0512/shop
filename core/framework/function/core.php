 ...
 * @return void
 */
function requestNotFound($con = null, $fun = null) {
 ...
 * @return string 字符串类型的返回结果
 */
function getNchash($con = '', $fun = ''){
    $con = $con ? $con : $_GET['con'];
    $fun = $fun ? $fun : $_GET['fun'];
    if (C('captcha_status_login')){
        return substr(md5(SHOP_SITE_URL.$con.$fun),0,8);
    } else {
        return '';
    }
}
...
* @return string	$price_format
*/
function ncPriceFormatForListsmall($price) {
    if ($price >= 10000) {
       return number_format(floor($price/100)/100,2,'.','').L('ten_thousand');
    } else {
     return $price;
 ...
 * @return int|null
 */
function getAppId($app_alias){
    if (is_string($app_alias) && !empty($app_alias)){
        switch ($app_alias){
            case 'hongmuzhijia':
                $appid = 2;
                break;
...
function getMemberAvatar($member_avatar){
    if (empty($member_avatar)) {
        return UPLOAD_SITE_URL.DS.'contacts'.DS.'default.png';
 ...
 * @return string
 */
function new_getMemberAvatar($hashkey){
    return "https://gateway.confolsc.com/avatar/{$hashkey}";
}
/**
 * 成员头像
    ...
    if ($type == 'store_avatar') {
            if (empty($img)) {
                ...
                if (stristr($img,'.')){
                    ...
                    return "https://gateway.confolsc.com/files/image/{$img}?redirect=1&type=1&size=200,200";
                }
            }
        ...
        if (empty($img)){
            return UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.C('app_alias').'_default_store_banner.jpg';
        } else {
            return UPLOAD_SITE_URL.DS.ATTACH_STORE.DS.$img;
        }
    }elseif ($type == 'card_logo'){
        if (empty($img)){
            return UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.'default_card_logo.jpeg';
        } else {
            return UPLOAD_SITE_URL.DS.ATTACH_AVATAR.DS.$img;
        }
    }
}
...
/**
 * 获取开店申请图片UPLOAD_SITE_URL.DS.ATTACH_STORE_JOININ
 */
function getStoreJoininImageUrl($image_name='default', $size=0) {
    if (empty($image_name))
        $image_name = 'default';
    return 'https://gateway.confolsc.com/files/image'.DS.$image_name."?redirect=1&size={$size},{$size}";
}
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('TTShop') or exit('Access Invalid!'); return ".$data.";";
...
function pagecmd($cmd ='', $arg = ''){
	if (!class_exists('page',false))	import('page');
	static $page;
	if ($page == null){
		$page = new Page();
	}
 ...
 * @return string
 */
function url($con = '', $fun = '', $args = array(), $model = false, $site_url = ''){
    //伪静态文件扩展名
    $ext = '.html';
    //入口文件名
    $file = 'index.php';
//    $site_url = empty($site_url) ? SHOP_SITE_URL : $site_url;
    ...
    //定义变量存放返回url
    $url_string = '';
    if (empty($con) && empty($fun) && empty($args)) {
        return $site_url;
    }
    $con = !empty($con) ? $con : 'index';
    $fun = !empty($fun) ? $fun : 'index';
    ...
    $reg_match_to = array(
        ...
        'glist-\\1-\\2-\\3-\\4-\\5-\\6.html',
        'gsoon-\\1-\\2-\\3-\\4-\\5-\\6.html',
        'ghistory-\\1-\\2-\\3-\\4-\\5-\\6.html',

        'vr_glist-\\1-\\2-\\3-\\4-\\5-\\6-\\7-\\8.html',
        'vr_gsoon-\\1-\\2-\\3-\\4-\\5-\\6-\\7-\\8.html',
        'vr_ghistory-\\1-\\2-\\3-\\4-\\5-\\6-\\7-\\8.html',
 ...
 * @return string
 */
function urlShop($con = '', $fun = '', $args = array(), $store_domain = ''){

    // 如何使自营店则返回javascript:;

//    if ($con == 'show_store' && $fun != 'goods_all') {
...
//    }
    ...
    if(URL_MODEL && in_array($con.':'.$fun, $rewrite_item)) {
        $rewrite_flag = true;
    ...
    return url($con, $fun, $args, $rewrite_flag, SHOP_SITE_URL.'/Merchant');
}

/**
 * 手机列表页url，强制使用动态传参数模式
 *
 * @param string $con control文件名
 * @param string $fun op方法名
 * @param array $args URL其它参数
...
function urlMobile($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, MOBILE_SITE_URL);
}
 ...
 * @return string
 */
function _formatPrice($price,$prefix='')
{
    //if ($price>=10000) {
    //    return $prefix.ncPriceFormatForListsmall($price);
    //} else {
        if (bccomp($price,'0.01',2)===0) {
            return '私洽';
        } else if (bccomp($price,'0.02',2)===0) {
            return '价格待询';
        } else {
...
function _out_formatPrice($price,$prefix = '￥')
{
    if (bccomp($price,'0.01',2)===0) {
        return '私洽';
    } else if (bccomp($price,'0.02',2)===0) {
 ...
 * @return string
 */
function urlAdmin($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, ADMIN_SITE_URL);
}
 ...
 * @return string
 */
function urlAdminShop($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, ADMIN_modules_URL);
}
/**
 ...
 * @return string
 */
function urlAdminCircle($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, CIRCLE_modules_URL);
}
/**
 ...
 */
function urlAdminMobile($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, MOBILE_modules_URL);
}
/**
 ...
 * @return stringurlAdminMobile
 */
function urlAdminCms($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, CMS_modules_URL);
}
/**
 ...
 * @return stringurlAdminMobile
 */
function urlAdminMicroshop($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, MICROSHOP_modules_URL);
}
/**
 ...
 * @return string
 */
function urlCMS($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, CMS_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlCircle($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, CIRCLE_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlMicroshop($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, MICROSHOP_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlMember($con = '', $fun = '', $args = array()) {
    // 默认标志为不开启伪静态
    $rewrite_flag = false;
    ...
    return url($con, $fun, $args, $rewrite_flag, MEMBER_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlLogin($con = '', $fun = '', $args = array()) {
    return url($con, $fun, $args, false, LOGIN_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlChain($con = '', $fun = '', $args = array()){
    return url($con, $fun, $args, false, CHAIN_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
