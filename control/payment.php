...
class paymentControl extends BaseHomeControl{
    ...
    public function real_orderOp(){
        ...
        if (empty($api_pay_amount)) {
            redirect(SHOP_SITE_URL.'/index.php?con=buy&fun=pay_ok&pay_sn='.$order_pay_info['data']['pay_sn'].'&pay_amount='.ncPriceFormat($pay_amount));
        }
    ...
    private function _api_pay($order_info, $payment_info) {
        ...
        if($payment_info['payment_code'] == 'jdpay') {
            $payment_api->submit();
