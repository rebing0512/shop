...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: 'index.php?con=web_channel&fun=code_update',
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load('index.php?con=web_channel&fun=recommend_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
