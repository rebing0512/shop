    ...
    public function article_class_addOp(){
        ...
        if (chksubmit()){
            ...
            $error = $obj_validate->validate();
                  //上传网站Logo
            if (!empty($_FILES['article_logo']['name'])){
                $upload = new UploadFile();
                $upload->set('default_dir',ATTACH_ARTICLE_LOGO);
                $result = $upload->upfile('article_logo');
                if ($result){
                    $_POST['article_logo'] = $upload->file_name;
                }else {
            ...
            if ($error != ''){
                ...
                $insert_array['ac_logo'] = trim($_POST['article_logo']);
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?con=article_class&fun=article_class_add&ac_parent_id='.intval($_POST['ac_parent_id']),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>'index.php?con=article_class&fun=article_class',
    ...
    public function article_class_editOp(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_FILES['article_logo']['name'])){
                ...
                $_POST['article_logo'] = $_POST['pic_ori'];
            }
            if ($error != ''){
                showMessage($error);
            }else {
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                $update_array['ac_logo'] =trim($_POST['article_logo']);
                $result = $model_class->updates($update_array);
                
                if ($result){
                       //判断有没有之前的图片，如果有则删除
                        if (!empty($class_array['article_logo']) && !empty($_POST['article_logo'])){
                            @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ARTICLE_LOGO.DS.$class_array['article_logo']);
                        }
                    $url = array(
                        array(
                            'url'=>'index.php?con=article_class&fun=article_class',
                        ...
                        array(
                            'url'=>'index.php?con=article_class&fun=article_class_edit&ac_id='.intval($_POST['ac_id']),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(l('nc_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],'index.php?con=article_class&fun=article_class');
                }else {
    ...
    public function article_class_delOp(){
        ...
        if (intval($_GET['ac_id']) > 0){
            ...
            showMessage($lang['article_class_index_del_succ'],'index.php?con=article_class&fun=article_class');
        }else {
            showMessage($lang['article_class_index_choose'],'index.php?con=article_class&fun=article_class');
        }
    }
    /**
     * ajax操作
