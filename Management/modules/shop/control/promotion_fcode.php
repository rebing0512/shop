...
class promotion_fcodeControl extends SystemControl{
    private $links = array(
        array('url'=>'con=promotion_fcode&fun=index','text'=>'商品列表'),
        array('url'=>'con=promotion_fcode&fun=fcode_quota_list','text'=>'套餐列表'),
        array('url'=>'con=promotion_fcode&fun=fcode_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?con=promotion_fcode&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
