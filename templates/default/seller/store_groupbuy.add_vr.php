...
<div class="ncsc-form-default">
  <form id="add_form" action="index.php?con=store_groupbuy&fun=groupbuy_save&vr=1" method="post" enctype="multipart/form-data">
    <dl>
    ...
    <dl>
      ...
      <dd>
          ...
          <p class="hint"><?php echo '特卖开始时间不能小于'.date('Y-m-d H:i', $output['groupbuy_start_time']);?></p>
      </dd>
    </dl>
    <dl>
...
<?php } ?>
    ...
    <dl>
      <dt><i class="required">*</i>特卖活动图片<?php echo $lang['nc_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt>特卖推荐位图片<?php echo $lang['nc_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt>特卖分类<?php echo $lang['nc_colon'];?></dt>
      <dd>
        ...
        <span></span>
        <p class="hint">请选择本次虚拟特卖所属分类</p>
      </dd>
    </dl>
    <dl>
      <dt>特卖区域<?php echo $lang['nc_colon'];?></dt>
      <dd>
    ...
    <dl>
      ...
      <dd>
        ...
        <span></span>
        <p class="hint">每个买家ID可特卖的最大数量，该数量不能大于虚拟商品本身的限购数量，不限数量请填"0"</p>
      </dd>
    </dl>
    <dl>
      ...
      <dd>
        ...
        <p class="hr8"><a class="des_demo ncbtn" href="index.php?con=store_album&fun=pic_list&item=groupbuy"><i class="icon-picture"></i><?php echo $lang['store_goods_album_insert_users_photo'];?></a></p>
...
$("select[name=class]").change(function(){
    ...
    $.ajax({
        type:'GET',
        url:'index.php?con=store_groupbuy&fun=ajax_vr_class&class_id='+class_id,
...
$("select[name=city]").change(function(){
    ...
    $.ajax({
        type:'GET',
        url:'index.php?con=store_groupbuy&fun=ajax_vr_area&area_id='+area_id,
        success:function(json){
            var html = '<option value="">'+'请选择...'+'</option>';
...
<?php } ?>
        ...
        messages : {
            ...
            start_time : {
                required : '<i class="icon-exclamation-sign"></i>特卖开始时间不能为空',
                greaterThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf('特卖开始时间必须大于{0}',date('Y-m-d H:i',$output['current_groupbuy_quota']['start_time']));?>'
            },
            end_time : {
                required : '<i class="icon-exclamation-sign"></i>特卖结束时间不能为空',
<?php if (!$output['isOwnShop']) { ?>
                lessThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf('特卖结束时间必须小于{0}',date('Y-m-d H:i',$output['current_groupbuy_quota']['end_time']));?>',
<?php } ?>
            ...
            upper_limit: {
                ...
                lessThanVrLimitNum : '虚拟特卖活动的限购数量不能大于虚拟商品本身的限购数量'
            },
            groupbuy_image: {
                required : '<i class="icon-exclamation-sign"></i>特卖图片不能为空'
            }
        }
    });
