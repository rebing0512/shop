...
class member_accountControl extends mobileMemberControl {
    ...
    */
    ...
    public function indexOp(){
         ...
         $store = Model('store')->where('member_id ='.$_SESSION['member_id'])->find();

         $store_avatar = getStoreLogo($store['store_avatar']);

         Tpl::output('store_info',$store);

         Tpl::output('member_info',$this->member_info);

         Tpl::output('store_avatar',$store_avatar);
    ...
    */
    ...
    public function update_imgOp(){
        ...
        if (!empty($_POST['touxiang'])){
            ...
            $handel = Model('store')->editStore(array('store_avatar'=>$_POST['touxiang']),array('store_id'=>$store_info['store_id']));

            if ($handel){
                echo json_encode([
                    'code' => 1,
                    'result' => $_POST['touxiang']
                ]);
                exit;
            } else {
                echo json_encode([
        ...
        $store = Model('store')->field('store_avatar')->where('member_id ='.$_SESSION['member_id'])->find();

        $store_avatar = getStoreLogo($store['store_avatar']);

        Tpl::output('store_avatar',$store_avatar);

        Tpl::output('store_ava',$store['store_avatar']);
    ...
    public function default_positionOp(){


        Tpl::output('web_seo',C('site_name').' - '.'设置默认中心');

        Tpl::output('default_position',$this->member_info['default_position']);

        Tpl::showpage('default_position');
    }

    public function update_defaultOp(){

        $default_position = trim($_POST['default_position']);

        $member_id = $_SESSION['member_id'];

        $model_member = Model('member');

        $update = $model_member->editMember(array('member_id'=>$member_id),array('default_position'=>$default_position));
...
//            $store_info = Model('store')->getStoreInfo($where);
            ...
            $wechatFileDownloadUrl = 'http://file.api.weixin.qq.com/cgi-bin/media/get?access_token=%s&media_id=%s';
            $wechatFileDownloadUrl = sprintf($wechatFileDownloadUrl,$this->sss(), $serverId);
            try {
                $ch = curl_init();
                $timeout = 5;
                curl_setopt($ch, CURLOPT_URL, $wechatFileDownloadUrl);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, $timeout);
                $contents = curl_exec($ch);
                curl_close($ch);

                //$path = tempnam(BASE_UPLOAD_PATH.DS.ATTACH_STORE,"TMP0");

                $path = BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.rand(10000,99999).'.jpeg';

                $file['list']=BASE_UPLOAD_PATH.DS.ATTACH_STORE;
                ...
                if (substr($response,0,1) == '{'){
                    ...
                    if (!$data)
                        throw new \Exception('下載遠程文件失敗，未知');
                    else
                        throw new \Exception($response);
                }

                $serverId = self::uploadToRemote($response,$path);
...
//                if ($serverId){
//                    Model('store')->editStore(array('store_avatar'=>$serverId),array('store_id'=>$store_info['store_id']));
//                }
    ...
    //              $type = $result[2];
    //
    //              $filename = "mobile_store_avatar_{$store_info['store_id']}.jpg";
    //
    //              file_put_contents($file['list'].'/'.$filename, base64_decode(str_replace($result[1], '', $img)));
    //
    //            }
    ...
    //            $member_avatar = $info[count($info)-1];
     ...
     //           $up = Model('store')->editStore(array('store_avatar'=>$filename),array('store_id'=>$store_info['store_id']));
    //            if($up){
