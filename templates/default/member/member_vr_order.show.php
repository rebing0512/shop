  ...
  <div class="ncm-order-info">
    <div class="ncm-order-details">
      ...
      <div class="content">
        ...
        <dl class="line">
          <dt>商&#12288;&#12288;家：</dt>
          <dd><?php echo $output['order_info']['store_name'];?><a href="javascript:void(0)" class="ncbtn" nc_type="dialog" dialog_width="800" dialog_title="查看地图" dialog_id="show_map" uri="index.php?con=show_map&fun=index&w=440&h=400&store_id=<?php echo $output['order_info']['store_id'];?>"><i class="icon-map-marker"></i>商家所在信息</a></dd>
        </dl>
      </div>
    </div>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?con=buy_virtual&fun=pay&order_id=<?php echo $output['order_info']['order_id']; ?>" class="ncbtn-mini ncbtn-bittersweet"><i></i>支付订单</a>以确保及时获取电子兑换码。</li>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="tl"><dl class="goods-name">
              ...
              <?php } ?>
              <dd><span class="sale-type"><?php if ($output['order_info']['order_promotion_type'] == 1) {?>特卖，<?php } ?>使用时效：即日起 至 <?php echo date("Y-m-d",$output['order_info']['vr_indate']);?>
          ...
          <td> <?php if($output['order_info']['if_refund']){ ?>
              <a href="index.php?con=member_vr_refund&fun=add_refund&order_id=<?php echo $output['order_info']['order_id']; ?>">退款</a>
              <?php } ?></td>
          <td class="bdl"><?php echo $output['order_info']['state_desc'];?></td>
          <td class="bdl bdr">

          <?php if ($output['order_info']['if_cancel']){ ?>
            <a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit" nc_type="dialog" dialog_width="480" dialog_title="取消订单" dialog_id="buyer_order_cancel_order" uri="index.php?con=member_vr_order&fun=change_state&state_type=order_cancel&order_id=<?php echo $output['order_info']['order_id'];?>"  id="order_action_cancel"><i class="icon-ban-circle"></i>取消</a>
            <?php }?>

            <!-- 评价 -->
            <?php if ($output['order_info']['if_evaluation']) { ?>
            <p><a class="ncbtn ncbtn-aqua" href="index.php?con=member_evaluate&fun=add_vr&order_id=<?php echo $output['order_info']['order_id']; ?>"><?php echo $lang['member_order_want_evaluate'];?></a></p>
            <?php } ?>
