...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        ...
        array('url'=>'con=store&fun=store_joinin','text'=>'认证申请'),
     ...
     *
     */
    public function url_copyOp(){
        Tpl::setDirquna('shop');
        Tpl::showpage('url_copy', 'null_layout');
    }
    ...
    public function get_xmlOp() {
        ...
        foreach ($store_list as $value) {
            ...
            $store_name = "<a class='" . $store_state . "' href='javascript:;' onclick='copyToClipBoard(".$value['store_id'].")'>";
            ...
            if($value['store_recommend'] == 1){
             $store_recommend= "<img src='/adminmin/templates/default/images/yes.png' class='table_list_toggle' list_id='".$value['store_id']."' list_val='1' list_key='store_recommend'>";
           }else{
             $store_recommend= "<img src='/adminmin/templates/default/images/no.png' class='table_list_toggle' list_id='".$value['store_id']."' list_val='0' list_key='store_recommend'>";
            }
            $store_name .= $value['store_name'] . "<i class='fa fa-external-link ' title='新窗口打开'></i></a>";
            $param['store_name'] = $store_name;
            $param['store_recommended'] = $store_recommend;
    ...
    */
    //ajax更新
   public function store_ajaxOp(){
       $where['store_id'] = $_POST['id'];

       $great_store = Model('great_store');

       $ns=$great_store->where($where)->find();

       if ($ns){
           $great_store->update($where);
       }

       $save[$_POST['key']] = $_POST['dst'];
    ...
    public function store_editOp(){
        ...
        if (chksubmit()){
            ...
            $update_array = array();
            $update_array['store_type'] = trim($_POST['store_type']);
            $update_array['store_name'] = trim($_POST['store_name']);

            if (!empty($_POST['businesscard_id'])){
                $card = Model('businesscard')->where('id ='.$_POST['businesscard_id'])->find();
                $card_id['businesscard_id'] = NULL;
                Model('store')->where('businesscard_id ='.$_POST['businesscard_id'])->update($card_id);
                if (empty($card)){
                    showMessage('不存在的名片');
                } else {
                    $update_array['businesscard_id'] = trim($_POST['businesscard_id']);
                    $data['store_id'] = $_GET['store_id'];
                    Model('businesscard')->where('id ='.$_POST['businesscard_id'])->update($data);
                }
            }
    ...
    public function shopwwi_addOp()
    {
        if (chksubmit())
        {
            ...
            $saveArray = array();
            $saveArray['store_type'] = $_POST['store_type'];
            $saveArray['store_name'] = $_POST['store_name'];
            $saveArray['businesscard_id'] = trim($_POST['businesscard_id']);
            ...
            showMessage('操作成功','index.php?con=store&fun=store');
            return;
        }
		Tpl::setDirquna('shop');
...
//        $condition['joinin_state'] = array('gt',0);
        ...
        $condition['store_type'] = ['in', [1, 3]];
        $order = '';
        $param = array('store_id', 'store_name', 'store_type', 'paying_amount', 'joinin_state', 'joinin_year', 'contacts_name', 'contacts_phone'
        ...
        //$fields = 'store.store_id,store.store_name,store.store_type,store_joinin.paying_amount,store_joinin.joinin_state,store_joinin.joinin_year,store_joinin.contacts_name,store_joinin.contacts_phone,store_joinin.contacts_email,store_joinin.company_name,store_joinin.company_province_id,store_joinin.company_phone,store_joinin.company_employee_count,store_joinin.company_registered_capital';
        $model_store_joinin = Model();
        $store_list = $model_store_joinin->table('store,store_joinin')->join('right')->on('store.member_id = store_joinin.member_id')->where($condition)->page($page?:'10')->select();
...
//            }
            ...
            $value['store_status'] = $this->apply_status[$value['store_type']];

            if ($value['store_type'] <= 1){
                $value['store_type'] = '普通店铺';
            } else if ($value['store_type'] <= 3&&$value['store_type'] > 1){
                $value['store_type'] = '认证店铺';
            } else {
                $value['store_type'] = '高级店铺';
            }

            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $param['store_name'] = $value['store_name'];
            $param['store_type'] = $value['store_type'];
            $param['store_status'] = $value['store_status'];
    ...
    public function verifyOp(){
        if ($_GET['id']&&$_GET['member_id']){
            switch ($_GET['id']){
                case 'btn_pass':
                    $handel = Db::query("UPDATE s_store SET store_type = store_type + 1,notice = 1,status = 0 WHERE member_id = {$_GET['member_id']}");
                    break;
                case 'btn_fail':
                    $handel = Db::query("UPDATE s_store SET store_type = store_type - 1,notice = 1,status = 1 WHERE member_id = {$_GET['member_id']}");
                    break;
            }
            if ($handel){
                showMessage('店铺申请审核完成','index.php?con=store&fun=store_joinin');
            } else {
                showMessage('店铺申请审核失败','index.php?con=store&fun=store_joinin');
            }
        } else {
            showMessage('参数错误','index.php?con=store&fun=store_joinin');
        }
    }
