...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"> <a class="back" href="javascript:history.go(-1);" title="返回"> <i class="fa fa-arrow-circle-o-left"></i> </a>
      <div class="subject">
        <h3>虚拟特卖 - 查看/编辑虚拟特卖区域“<?php echo $output['parent_area']['area_name']; ?>”下级区域</h3>
        <h5>商家可设置其虚拟特卖活动的区域以便于会员检索</h5>
      </div>
    </div>
  </div>
    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr data-id="<?php echo $val['area_id'] ;?>">
          ...
          <td class="handle"><a href="index.php?con=vr_groupbuy&fun=area_street&parent_area_id=<?php echo $val['area_id']; ?>" class="btn green"><i class="fa fa-list-alt"></i>查看</a> <span class="btn"> <em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em>
            <ul>
              <li><a href="index.php?con=vr_groupbuy&fun=area_edit&area_id=<?php echo $val['area_id']; ?>">编辑商区</a></li>
        ...
        buttons: [
            {
                ...
                title: '新增商区',
                onpress: function() {
                    location.href = 'index.php?con=vr_groupbuy&fun=area_add&area_id=<?php echo $_GET['parent_area_id']; ?>';
                }
            },
            {
...
function submit_delete(id){
    if (confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        ...
        $('#list_form').attr('action','index.php?con=vr_groupbuy&fun=area_drop');
        $('#area_id').val(id);
        $('#list_form').submit();
    }
}
