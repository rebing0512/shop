...
class store_returnControl extends BaseSellerControl {
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            $reload = 'index.php?con=store_return&lock=1';
            if ($return['order_lock'] == '2') {
                $reload = 'index.php?con=store_return&lock=2';
            }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'return':
                $menu_array = array(
                    array('menu_key'=>'2','menu_name'=>'售前退货',  'menu_url'=>'index.php?con=store_return&lock=2'),
                    array('menu_key'=>'1','menu_name'=>'售后退货','menu_url'=>'index.php?con=store_return&lock=1')
                );
                break;
        }
