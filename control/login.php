...
class loginControl extends BaseLoginControl {
	...
	public function logoutOp() {
		...
		if (C('ucenter_status')) {
			...
			showMessage($lang['login_logout_success'], 'index.php?con=login&ref_url=' . urlencode($ref_url));
		} else {
			redirect();
		}
	}
	...
	public function usersaveOp() {
		...
		if (!isset($member_info['error'])) {
			...
			$_POST['ref_url'] = (strstr($_POST['ref_url'], 'logout') === false && !empty($_POST['ref_url']) ? $_POST['ref_url'] : urlShop('member_information', 'member'));
			if ($_GET['inajax'] == 1) {
				showDialog('', $_POST['ref_url'] == '' ? 'reload' : $_POST['ref_url'], 'js');
			} else {
				redirect($_POST['ref_url']);
	...
	public function find_passwordOp() {
		...
		//Lib::messager()->send($_POST["email"],$subject,$message);
		if ($result) {
			showDialog('新密码已经发送至您的邮箱，请尽快登录并更改密码！', '', 'succ', '', 5);
		} else {
			showDialog('发送失败！', '', 'error', '', 3);
		}
	...
	public function bind_emailOp() {
		...
		showMessage('邮箱设置成功', 'index.php?con=member_security&fun=index');
