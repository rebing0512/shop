...
class advControl extends SystemControl{
    ...
    public function adv_addOp(){
        if(!chksubmit()){
            ...
            if ($error != ''){
                ...
                $insert_array['adv_summary'] = trim($_POST['adv_summary']);
                /**
                 * 建立文字广告信息的入库数组
                 */
                //判断页面编码确定汉字所占字节数
                ...
                //图片广告
                if($_POST['aptype_hidden'] == '0' || $_POST['aptype_hidden'] == '4'|| $_POST['aptype_hidden'] == '5'){
                ...
                if ($result&&$result2){
                    ...
                    showMessage($lang['adv_add_succ'],'index.php?con=adv&fun=adv&ap_id='.$_POST['ap_id']);
                }else {
                    showMessage($lang['adv_add_fail'],'index.php?con=adv&fun=adv&ap_id='.$_POST['ap_id']);
                }
        }
      }
    }
    ...
    public function get_ap_xmlOp(){
        ...
        foreach ((array)$ap_list as $k => $ap_info) {
            ...
            $operation_detail = "<li><a href=\"index.php?con=adv&fun=adv&ap_id={$ap_info['ap_id']}\"></i>管理广告</a></li>";
            $operation_detail .= "<li><a href='index.php?con=adv&fun=ap_edit&ap_id={$ap_info['ap_id']}'>编辑属性</a></li>";
            ...
            $list['ap_class'] = str_replace(array(0,1,3,4,5), array('图片','文字','Flash','多图轮播','wap端多图轮播'),$ap_info['ap_class']);
        ...
        if(!chksubmit()){
            ...
            if ($error != ''){
                ...
                if($_POST["is_use"] != ''){
                  ...
                  if($_POST['ap_setting_anniu_wenzi'] != ''){
                    $param['ap_setting_anniu_wenzi'] = intval(trim($_POST['ap_setting_anniu_wenzi']));
                    $param['ap_setting_anniu_weizhi'] = trim($_POST['ap_setting_anniu_weizhi']);
                    $param['ap_setting_anniu_ys'] = trim($_POST['ap_setting_anniu_ys']);
                    $param['ap_setting_anniu_ys_after'] = trim($_POST['ap_setting_anniu_ys_after']);
    ...
    public function ap_addOp(){
        if($_POST['form_submit'] != 'ok'){
            ...
            if ($error != ''){
                ...
                if($_POST['ap_setting_anniu_wenzi'] != ''){
                    $insert_array['ap_setting_anniu_wenzi'] = intval(trim($_POST['ap_setting_anniu_wenzi']));
                    $insert_array['ap_setting_anniu_weizhi'] = trim($_POST['ap_setting_anniu_weizhi']);
                    $insert_array['ap_setting_anniu_ys'] = trim($_POST['ap_setting_anniu_ys']);
                    $insert_array['ap_setting_anniu_ys_after'] = trim($_POST['ap_setting_anniu_ys_after']);
                ...
                if ($result){
                    ...
                    showMessage($lang['ap_add_succ'],'index.php?con=adv&fun=ap_manage','html','succ',1,4000);
                }else {
    ...
    public function get_adv_xmlOp(){
        ...
        foreach ((array)$adv_list as $k => $adv_info) {
            $list = array();$operation_detail = '';
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$adv_info['adv_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?con=adv&fun=adv_edit&adv_id={$adv_info['adv_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            ...
            foreach ($ap_list as $ap_k => $ap_v){
                if($adv_info['ap_id'] == $ap_v['ap_id']){
                    ...
                    $list['ap_class'] = str_replace(array(0,1,3,4,5), array('图片','文字','Flash','多图轮播','wap端多图轮播'),$ap_v['ap_class']);
                    break;
                }
            }
    ...
    public function adv_editOp(){
        ...
        if($_POST['form_submit'] != 'ok'){
            ...
            if ($error != ''){
                ...
                $param['adv_summary'] = trim($_POST['adv_summary']);
