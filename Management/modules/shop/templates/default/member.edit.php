...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?con=member&fun=member" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
  ...
  <form id="user_form" enctype="multipart/form-data" method="post">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dl class="row">
            <dt class="tit">
                <label><?php echo 'VIP';?></label>
            </dt>
            <dd class="opt">
                <input type="radio" <?php if($output['member_array']['vip'] == 0){ ?>checked="checked"<?php } ?> value="0" name="vip" id="vip0">
                <label for="vip0"><?php echo '否';?></label>
                <input type="radio" <?php if($output['member_array']['vip'] == 1){ ?>checked="checked"<?php } ?> value="1" name="vip" id="vip1">
                <label for="vip1"><?php echo '是';?></label>
                <span class="err"></span> </dd>
        </dl>
      <dl class="row">
        <dt class="tit">
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?con=common&fun=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR;?>/avatar_<?php echo $_GET['member_id'];?>.jpg&url='+data.url,690);
					}else{
						alert(data.msg);
					}
