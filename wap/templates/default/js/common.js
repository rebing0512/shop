...
var SiteUrl = window.location.protocol + "//" + window.location.host + "/wap";
var ApiUrl = window.location.protocol + "//" + window.location.host + "/wap";
var Market_ApiUrl = window.location.protocol + "//" + window.location.host + "/wap/index.php?con=goods&fun=supermarket";
var ShopUrl = window.location.protocol + "//" + window.location.host + "";
var CategoryUrl = window.location.protocol + "//" + window.location.host + "/wap/index.php?con=category&fun=index&ref=search";
var pagesize = 20;
var WapSiteUrl = window.location.protocol + "//" + window.location.host + "/wap";
// var IOSSiteUrl = "https://itunes.apple.com/us/app/";
// var AndroidSiteUrl = "https://www.ynlmsc.pw//download/app/1.apk";
...
function get_share_url(e) {
    var url = encodeURIComponent(e);
    if (url) {
        var share_url = ApiUrl + '?con=api&fun=share&redirect_url=' + url;
        return share_url;
    } else {
        return "";
    }
...
function checkLogin(e) {
    ...
    if (e == 0) {
        ...
        location.href = ApiUrl + "/index.php?con=login";
...
var TipMsg = {
    ...
    position: function (msg, tag, timer, leftplus, topplus, direction) {/*"提示",$(this),2000,向左偏移,向上偏移,方向*/
        ...
        if (_direction == "up") {
            ...
            the.css({
                top: tagOff.top + topplus,
                left: tagOff.left + tag.outerWidth() + 10 + leftplus
            }).addClass("rightTip");
        ...
        if (timer != -1) {
            ...
            the.hover(function () {
                clearTimeout(TipMsg.showTimer);
            }, function () {

                TipMsg.showTimer = setTimeout(function () {

                    the.fadeOut(300, function () {
                        the.remove();
                    });
...
function showtip(msg, obj, opt, time) {
    ...
    if (!wintip) {
        ...
        wintip.posshow = function (time, posobj) {
            ...
            wintip.timer = setTimeout(function () {
                $("#tipdiv").remove();
            }, time);
        ...
        wintip.setmsg = function (msg) {
            this.msgobj.innerHTML = msg;
            return this;
        }
