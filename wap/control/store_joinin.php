...
//        }
    ...
    public function step1Op() {
        ...
        $conditon['member_id'] = $_SESSION['member_id'];

        $info = $model->where($conditon)->find();

        $store_info = Model('store')->where($conditon)->find();

        Tpl::output('store_info',$store_info);

        Tpl::output('info',$info);

        Tpl::output('web_seo','店铺资质信息');
    ...
    public function savestepOp(){
       ...
       if (!$_SESSION['member_id']){
           output_data([
               'url' => 'https://gateway.confolsc.com/passport/login?appid='.C('app_alias').'&redirect='.urlencode(urlMobile('store_joinin', 'step1'))
           ]);
       }

        $store = Model('store')->where('member_id = '.$_SESSION['member_id'])->find();

        if (in_array($store['store_type'],array(1,3))){
            output_data('审核中，无法修改提交资料');
            exit;
        }
         ...
         if(!empty($_POST)) {
            ...
            $param['company_name'] = $_POST['company_name'];
             ...
             $param['company_employee_count'] = $_POST['company_employee_count'];

             $param['company_registered_capital'] = $_POST['company_registered_capital'];
            ...
            Db::query("UPDATE s_store SET store_type = store_type + 1,notice = 1 WHERE member_id = {$_SESSION['member_id']}");
            ...
            if(empty($joinin_info)) {
                ...
                output_data(array('url'=>urlMobile('member_account')));
...
//            array("input"=>$param['company_address'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"店铺地址不能为空且必须小于50个字"),

            array("input"=>$param['company_address_detail'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"店铺详细地址不能为空且必须小于50个字"),

            array("input"=>$param['contacts_name'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"20","message"=>"店主姓名不能为空且必须小于20个字"),

            array("input"=>$param['contacts_phone'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"20","message"=>"联系号码不能为空"),
...
//            array("input"=>$param['company_employee_count'], "require"=>"true","message"=>"员工数量不能为空"),
//
//            array("input"=>$param['company_registered_capital'], "require"=>"true","message"=>"注册资金不能为空"),
//
//            array("input"=>$param['business_licence_number'], "require"=>"true","message"=>"营业执照号不能为空"),
//
//            array("input"=>$param['business_licence_address'], "require"=>"true","message"=>"营业执照所在地不能为空"),
//
//            array("input"=>$param['business_sphere'], "require"=>"true","message"=>"营业执照所在地不能为空"),
