...
class chat_logControl extends SystemControl{
    ...
    public function get_msg_xmlOp() {
        ...
        foreach ($log_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn green' href='index.php?con=chat_log&fun=chat_log&f_name=".$value['f_name']."&t_name=".$value['t_name']."&add_time_from=".date("Y-m-d",$value['add_time'])."&add_time_to=".date("Y-m-d",$value['add_time'])."' class='url'><i class='fa fa-list-alt'></i>详情</a>";
            $param['t_msg'] = $value['t_msg'];
            $param['add_time'] = date('Y-m-d H:i:s', $value['add_time']);
            $param['f_name'] = $value['f_name'];
            $param['f_id'] = $value['f_id'];
