...
class mb_specialControl extends SystemControl{
    ...
    public function index_editOp() {
        ...
        Tpl::output('page', $model_mb_special->showpage(2));
        // p($model_mb_special->getMbSpecialModuleList());die;
    ...
    public function special_item_addOp() {
        ...
        $param['goods_type'] = $_POST['goods_type'];
        //广告只能添加一个
        ...
        //页面导航只能添加一个
        if($param['item_type'] == 'nav') {

            $result = $model_mb_special->isMbSpecialItemExist($param);
            if($result) {
                echo json_encode(array('error' => '页面导航只能添加一个'));die;
            }
        }
    ...
    //特卖只能添加一个
        if($param['item_type'] == 'goods2') {
            $result = $model_mb_special->isMbSpecialItemExist($param);
            if($result) {
                echo json_encode(array('error' => '特卖板块只能添加一个'));die;
            }
        }
    ...
    public function special_item_editOp() {
        ...
        if($goods_type==1){
            ...
            Tpl::showpage('mb_special_item.edit3');
        }else{
