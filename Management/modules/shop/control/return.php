...
class returnControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'con=return&fun=return_manage','text'=>'待处理'),
            array('url'=>'con=return&fun=return_all','text'=>'所有记录')
    );
    ...
    public function get_manage_xmlOp() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?con=return&fun=edit&return_id={$return_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function get_all_xmlOp() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"index.php?con=return&fun=view&return_id={$return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?con=return&fun=return_manage');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
