...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?con=circle_manage&fun=circle_list" title="返回圈子列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'<?php echo ADMIN_SITE_URL?>/index.php?con=common&fun=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_CIRCLE;?>/group',
				secureuri:false,
				fileElementId:'_pic',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?con=common&fun=pic_cut&type=circle&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/'.ATTACH_CIRCLE;?>/group/<?php echo $_GET['c_id'];?>.jpg&url='+data.url,690);
					}else{
						alert(data.msg);
					}
	...
	$('#mastername').focus(function(){
		ajax_form('choose_master', '<?php echo $lang['circle_choose_master'];?>', 'index.php?con=circle_manage&fun=choose_master&c_id=<?php echo $output['circle_info']['circle_id'];?>', 640);
	});
	...
	$('#circle_form').validate({
        ...
        rules : {
        	c_name : {
        		...
        		maxlength : 12,
            	remote : {
            		url : 'index.php?con=circle_manage&fun=check_circle_name&id=<?php echo $output['circle_info']['circle_id'];?>',
                    type: 'get',
                    data:{
                    	name : function(){
                            return $('#c_name').val();
            ...
            mastername : {
            	required : true,
                remote   : {
                    url :'index.php?con=circle_manage&fun=check_member&c_id=<?php echo $output['circle_info']['circle_id'];?>&branch=ok',
                    type:'get',
                    data:{
                        name : function(){
