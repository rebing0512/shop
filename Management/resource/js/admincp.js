...
//布局换色设置
    ...
    $('div[data-param^="map-"]').find('i').click(function(){
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON('index.php?con=common&fun=common_operations', {type : 'del', value : _value}, function(data){
                if (data) {
            ...
            $.getJSON('index.php?con=common&fun=common_operations', {type : 'add', value : _value}, function(data){
                if (data) {
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : 'index.php?con=common&fun=ajax_pending_matters',
                async : false,
    ...
    if (_commonPendingMatters > 0) {
        $('li[nctype="pending_matters"]').show().find('em').html(parseInt($.cookie('commonPendingMatters')));
    }
});
...
function openItem(param) {
    ...
    $('#workspace').attr('src', ADMIN_SITE_URL + '/modules/' + data_str[0] + '/index.php?con=' + data_str[1]);
