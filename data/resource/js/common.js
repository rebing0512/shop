...
function login_dialog(){
    CUR_DIALOG = ajax_form('login','登录',SHOP_SITE_URL+'/index.php?con=login&inajax=1',360,1);
}
...
function collect_store(fav_id,jstype,jsobj){
    $.get('index.php?con=index&fun=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?con=member_favorite_store&fun=favoritestore';
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('index.php?con=index&fun=login', function(result){
        if(result=='0'){
            ...
            var url = 'index.php?con=member_favorite_goods&fun=favoritegoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
                showDialog(data.msg, 'succ','','','','','','','','',2);
...
function load_cart_information(){
	$.getJSON(SITEURL+'/index.php?con=cart&fun=ajax_load&callback=?', function(result){
	    var obj = $('.minicart_list');
	    if(result){
	       	var html = '';
                html+='<ul>';
	       	if(result.cart_goods_num >0){
	            for (var i = 0; i < result['list'].length; i++){
                    ...
                    html+='<li nctpye="cart_item_'+goods['cart_id']+'"><a traget="_blank" class="pro_img" href="'+goods['goods_url']+'" title="'+goods['goods_name']+'">';
                    html+=' <img src="'+goods['goods_image']+'"></a>';
                    html+='<a traget="_blank" class="pro_name" href="'+goods['goods_url']+'">'+goods['goods_name']+'</a>';
                    html+='<div class="mcartRight"><span class="pro_price"><em>&yen;'+goods['goods_price']+'×'+goods['goods_num']+'</em></span><em><a href="javascript:void(0);" onclick="drop_topcart_item('+goods['cart_id']+');">删除</a></em>';
                    html+='</div></li>';
		        }
                html +='</ul>';
		        html +='<div class="checkout_box">';
                html +='<p class="fl">共<em class="tNum">'+result.cart_goods_num+'</em>种商品,总计金额：<em class="tSum">&yen;'+result.cart_all_price+'</em></p>'
                html +='<a rel="nofollow" class="checkout_btn" href="/index.php?con=cart" target="_self"> 去结算 </a>'
                html +='</div>';
                $('.mod_minicart').find('.cart_num').html(result.cart_goods_num);
                obj.find('.list_detail').html(html);
    
	      } else {
	      	html="<div class='none_tips'><i> </i><p>购物车中没有商品，赶紧去选购！</p></div>";
...
function drop_topcart_item(cart_id){
    $.getJSON(SITEURL+'/index.php?con=cart&fun=del&cart_id='+cart_id+'&callback=?', function(result){
        if(result.state){
            ...
            if(result.quantity == 0){
                ...
                if ($('#rtoolbar_cartlist > ul').children().size() != result.quantity) {
                    $("#rtoolbar_cartlist").load(SHOP_SITE_URL+ '/index.php?con=cart&fun=ajax_load&type=html');return ;
                }
            }
        }else{
            alert(result.msg);
...
function load_history_information(){
    $.getJSON(SITEURL+'/index.php?con=index&fun=viewed_info', function(result){
...
(function($) {
    $.show_nc_login = function(options) {
        var settings = $.extend({}, {action:'/index.php?con=login&fun=login&inajax=1', nchash:'', formhash:'' ,anchor:''}, options);
        var login_dialog_html = $('<div class="quick-login"></div>');
        var ref_url = document.location.href;
        var add_html = '<span class="other">';
        if (connect_qq == '1'){
            add_html += '<a href="'+SHOP_SITE_URL+'/index.php?con=connect_qq" title="QQ账号登录" class="qq"><i></i></a>';
        }
        if (connect_sn == '1'){
            add_html += '<a href="'+SHOP_SITE_URL+'/index.php?con=connect_sina" title="新浪微博账号登录" class="sina"><i></i></a>';
        }
        if (connect_wx == '1'){
            add_html += '<a href="javascript:void(0);" onclick="ajax_form(\'weixin_form\', \'微信账号登录\', \''+LOGIN_SITE_URL+'/index.php?con=connect_wx&fun=index\', 360);" title="微信账号登录" class="wx"><i></i></a>';
        }
        add_html += '</span>';
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+LOGIN_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'index.php?con=seccode&amp;fun=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="index.php?con=seccode&amp;fun=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>')
        	.append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="'+LOGIN_SITE_URL+'/index.php?con=login&amp;fun=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+LOGIN_SITE_URL+'/index.php?con=login&amp;fun=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
...
(function($){
	$.fn.membershipCard = function(options){
		...
		return this.each(function(){
			...
			var _uri = SITEURL+'/index.php?con=member_card&callback=?&uid='+data_str.id+'&from='+options.type;
			var _dl = '';
			$this.qtip({
	            content: {
	                text: 'Loading...',
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
	                			...
	                			.append('<a href="'+SHOP_SITE_URL+'/index.php?con=member_snshome&mid='+data.id+'" target="_blank">'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''));
		                		//show membergrade
		                		...
		                		// avatar
		                		$('<dd class="avatar"><a href="'+SHOP_SITE_URL+'/index.php?con=member_snshome&mid='+data.id+'" target="_blank"><img src="'+data.avatar+'" /></a><dd>')
		                			.appendTo(_dl);
		                		// info
		                		var _info = '';
		                		...
		                		// ajax info
		                		if(data.url != ''){
			                		$.getJSON(data.url+'/index.php?con=member_card&fun=mcard_info&uid='+data.id, function(d){
			                			if(d){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+SHOP_SITE_URL+'/index.php?con=member_message&fun=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(SHOP_SITE_URL+'/index.php?con=member_snsfriend&fun=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( "data_str = "+data_str);
			$.getJSON(SHOP_SITE_URL+'/index.php?con=member_snsfriend&fun=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function circle_function(d){
			...
			$.each(d,function(i, n){
				rs.append('<div class="rank-div" title="'+n.circle_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'"><a href="'+CIRCLE_SITE_URL+'/index.php?con=group&c_id='+n.circle_id+'" target="_blank">'+n.circle_name+'</a><i></i><em class="rank-em rank-'+n.cm_level+'">'+n.cm_level+'</em></div>');
			});
			return rs;
		}
	...
	$.fn.nc_region = function(options) {
		...
		function loadAreaArray(callback) {
			if (typeof nc_a === "undefined") {
				$.getJSON(SITEURL + "/index.php?con=index&fun=json_area&src=" + settings.src + "&callback=?", function(data) {
					nc_a = data;
					callback()
				})
			} else {
...
function addcart(goods_id,quantity,callbackfunc) {
    ...
    var url = 'index.php?con=cart&fun=add';
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $("#rtoolbar_cartlist").load(SHOP_SITE_URL + '/index.php?con=cart&fun=ajax_load&type=html');
            } else {
                alert(data.msg);
            }
        }
...
function autoBanner(a, b,cur,timer,type,after,next) {
    ...
    if(type ){
        ...
        function play1() {
            if (curi1 >= a.length) { curi1 = 0 }
            a.eq(curi1).fadeIn(500).css({'display':'inline-block'}).siblings().fadeOut(600);
            
            b.eq(curi1).fadeIn(500).siblings().fadeOut(600);
            curi1++;
        }
        autoPlay1 = setInterval(function () { play1(); }, timer);
    }else{
            var curi = 1;
            var autoPlay;
            a.eq(0).addClass(cur);
            ...
            a.mouseover(function () {
                $(this).css({'background':after,'border':'1px solid '+after}).siblings().css({'background':next,'border':'1px solid '+next});
                var i = curi = $(this).index();
                b.eq(i).fadeIn(500).siblings().fadeOut(600);
            });
            function play() {
                if (curi >= a.length) { curi = 0 }
                a.eq(curi).css({'background':after,'border':'1px solid '+after}).siblings().css({'background':next,'border':'1px solid '+next});
                b.eq(curi).fadeIn(500).siblings().fadeOut(600);
                curi++;
            }
            autoPlay = setInterval(function () { play(); }, timer);
            b.hover(function () { clearInterval(autoPlay) }, function () { autoPlay = setInterval(function () { play(); }, timer); }); 
