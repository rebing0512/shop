...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=adv&fun=get_ap_xml',
        colModel : [
...
function fg_delete(id){
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?con=adv&fun=delete",
        data: "del_id="+id,
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?con=adv&fun=ap_add';
    }else if (name == 'delete') { 
...
{
   ajax_form('copy_adv', '<?php echo $lang['ap_get_js'];?>', 'index.php?con=adv&fun=ap_copy&id='+id);
}
</script> 
