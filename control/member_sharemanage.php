...
class member_sharemanageControl extends BaseMemberOneControl {
    public function __construct(){
        parent::__construct();
        Language::read('member_sharemanage');
        //判断系统是否开启站外分享功能
        if (C('share_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?con=member_information&fun=member','html','error');
        }
    }
    /**
     * 分享绑定列表
    ...
    public function unbindOp(){
        $type = $_GET['type'];
        if (empty($type)){
            showDialog(Language::get('wrong_argument'),'index.php?con=member_sharemanage','error');
        }
        $model = Model('sns_binding');
    ...
    public function share_qqzoneOp(){
        ...
        if (C('share_qqzone_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?con=member_sharemanage','html','error');
        }
