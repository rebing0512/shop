...
$(function(){
    ...
    $('#zomslide_color').colorpicker({showOn:'both'});//初始化切换大图背景颜色控件
    $('#zomslide_color').parent().css("width",'');
    $('#zomslide_color').parent().addClass("color");
    ...
    zomslide_obj = $("#upload_zomslide_form");
    ap_obj = $("#ap_screen");
    ap_zomslide_obj = $("#ap_zomslide");
    upload_obj = $("#upload_screen");
    upload_zomslideobj = $("#upload_zomslide");
	screen_obj.find("ul").sortable({ items: 'li' });
    zomslide_obj.find("ul").sortable({ items: 'li' });
...
//焦点区切换大图上传
function add_zomslide(add_type) {//增加图片

    for (var i = 1; i <= screen_max; i++) {//防止数组下标重复

        if (zomslide_obj.find("li[zomslide_id='"+i+"']").size()==0) {//编号不存在时添加
            var text_input = '';
            var text_type = '图片调用';
            var ap = 0;
            text_input += '<input name="zomslide_list['+i+'][pic_id]" value="'+i+'" type="hidden">';
            text_input += '<input name="zomslide_list['+i+'][pic_name]" value="" type="hidden">';
            if(add_type == 'adv') {
                ap = 1;
                text_type = '广告调用';
                text_input += '<input name="zomslide_list['+i+'][ap_id]" value="" type="hidden">';
            } else {
                text_input += '<input name="zomslide_list['+i+'][pic_url]" value="" type="hidden">';
                text_input += '<input name="zomslide_list['+i+'][mob_pic_url]" value="" type="hidden">';
            }
            text_input += '<input name="zomslide_list['+i+'][color]" value="" type="hidden">';
            text_input += '<input name="zomslide_list['+i+'][pic_img]" value="" type="hidden">';
            var add_html = '';
            add_html = '<li ap="'+ap+'" zomslide_id="'+i+'" title="可上下拖拽更改显示顺序"><div class="title"><h4>'+text_type+
            '</h4><a class="ncap-btn-mini del" href="JavaScript:del_zomslide('+i+
            ');"><i class="fa fa-trash"></i>删除</a></div><div class="focus-thumb" onclick="select_zomslide('+i+');" title="点击编辑选中区域内容"><img src="'+ADMIN_TEMPLATES_URL
            +'/images/picture.gif" /></div>'+text_input+'</li>';
            zomslide_obj.find("ul").append(add_html);
            select_zomslide(i);
            break;
        }
    }
...
function zomslide_pic(pic_id,pic_img) {//更新图片

    if (pic_img!='') {
        var color = zomslide_obj.find("input[name='zomslide_pic[color]']").val();
        var pic_name = zomslide_obj.find("input[name='zomslide_pic[pic_name]']").val();
        var pic_url = zomslide_obj.find("input[name='zomslide_pic[pic_url]']").val();
        var obj = zomslide_obj.find("li[zomslide_id='"+pic_id+"']");
        obj.find("img").attr("src",UPLOAD_SITE_URL+'/'+pic_img);
        obj.find("img").attr("title",pic_name);
        obj.find("input[name='zomslide_list["+pic_id+"][pic_name]']").val(pic_name);
        obj.find("input[name='zomslide_list["+pic_id+"][pic_url]']").val(pic_url);
        obj.find("input[name='zomslide_list["+pic_id+"][color]']").val(color);
        obj.find("input[name='zomslide_list["+pic_id+"][pic_img]']").val(pic_img);
        obj.find(".focus-thumb").css("background-color",color);
    }
    zomslide_obj.find('.web-save-succ').show();
    setTimeout("zomslide_obj.find('.web-save-succ').hide()",2000);
}
function zomslide_ap(pic_id,color) {//更新广告位
    var obj = zomslide_obj.find("li[screen_id='"+pic_id+"']");
    obj.find(".focus-thumb").css("background-color",color);
    zomslide_obj.find('.web-save-succ').show();
    setTimeout("zomslide_obj.find('.web-save-succ').hide()",2000);
}
...
function select_zomslide(pic_id) {//选中图片
    var obj = zomslide_obj.find("li[zomslide_id='"+pic_id+"']");

    var ap = obj.attr("ap");
    zomslide_obj.find("li").removeClass("selected");
    zomslide_obj.find("input[name='key']").val(pic_id);
    obj.addClass("selected");
    if(ap == '1') {
        upload_zomslideobj.hide();
        zomslide_obj.find("input[name='ap_pic_id']").val(pic_id);
        var a_id = obj.find("input[name='zomslide_list["+pic_id+"][ap_id]']").val();
        if(a_id == '') {//未选择广告位时用默认的
            $("#ap_id_screen").trigger("onchange");
        } else {
            var color = obj.find("input[name='zomslide_list["+pic_id+"][color]']").val();
            $("#ap_id_screen").val(a_id);
            $("#ap_color").val(color);
            ap_zomslide_obj.find('.evo-pointer').css("background-color",color);
        }
        ap_zomslide_obj.show();
    } else {
        ap_zomslide_obj.hide();
        var pic_name = obj.find("input[name='zomslide_list["+pic_id+"][pic_name]']").val();
        var pic_url = obj.find("input[name='zomslide_list["+pic_id+"][pic_url]']").val();
        var mob_pic_url = obj.find("input[name='zomslide_list["+pic_id+"][mob_pic_url]']").val();
        var color = obj.find("input[name='zomslide_list["+pic_id+"][color]']").val();
        $("input[name='zomslide_id']").val(pic_id);
        $("input[name='zomslide_pic[pic_name]']").val(pic_name);
        $("input[name='zomslide_pic[pic_url]']").val(pic_url);
        $("input[name='zomslide_pic[mob_pic_url]']").val(mob_pic_url);
        $("input[name='zomslide_pic[color]']").val(color);
        upload_zomslideobj.find(".type-file-file").val('');
        upload_zomslideobj.find(".type-file-text").val('');
        upload_zomslideobj.show();
        upload_zomslideobj.find('.evo-pointer').css("background-color",color);
    }
}
...
function select_ap_screen() {//选择广告位
    ...
    if (ap_id > 0 && typeof zomslide_adv_list[ap_id] !== "undefined") {
        var adv = zomslide_adv_list[ap_id];
	    var color = $("#ap_color").val();
	    var pic_name = adv['ap_name'];
	    var pic_img = adv['ap_img'];
	    var obj = zomslide_obj.find("li.selected");
...
function del_zomslide(pic_id) {//删除图片
    if (zomslide_obj.find("li").size()<2) {
         return;//保留一个
    }
    zomslide_obj.find("li[zomslide_id='"+pic_id+"']").remove();
    var slide_id = zomslide_obj.find("input[name='key']").val();
    if (pic_id==slide_id) {
        zomslide_obj.find("input[name='key']").val('');
        ap_obj.hide();
        upload_zomslideobj.hide();
    }
}
