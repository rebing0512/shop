...
<div class="wrap">
  ...
  <div class="ncm-default-form">
    <form method="post" id="goods_form" action="index.php?con=member_flea&fun=<?php if ($output['goods']['goods_id']!='') echo 'edit_save_goods'; else echo 'save_goods'; ?>">
                  ...
                  <ul id="goods_images">
                    ...
                    <li nc_type="handle_pic" file_id="<?php echo $output['goods_image'][$i]['upload_id']; ?>" thumbnail="<?php echo $output['goods_image'][$i]['file_thumb']; ?>" id="thumbnail_<?php echo $i;?>">
                      ...
                      <div class="upload-btn"><a href="javascript:void(0);">
                        <iframe id="iframe_<?php echo $i;?>" width="86" scrolling="no" height="30" frameborder="0" src="index.php?con=member_flea&fun=image_upload&id=<?php echo $i;?>&item_id=<?php echo $output['item_id']?>&file_id=<?php echo $output['goods_image'][$i]['upload_id']; ?>" ></iframe>
                        </a></div>
                    </li>
                    <?php } ?>
                  </ul>
...
<?php showEditor('g_body',$output['goods']['goods_body'],'800px','480px','visibility:hidden;',"false",$output['editor_multimedia']);?>
          ...
          <p class="info-album"><a class="des_demo" href="index.php?con=flea_album&fun=pic_list&item=des&goods_id=<?php echo $_GET['goods_id']; ?>"><?php echo $lang['store_goods_album_insert_users_flea_photo'];?></a></p>
...
function add_uploadedfile(file_data)
{
  ...
  if(file_data.state == 'false') {
    ...
    if(file_data.instance == 'goods_image'){
        ...
        $('#iframe_'+id).attr('src','index.php?con=member_flea&fun=image_upload&id='+id+'&item_id='+item_id+'&file_id='+file_data.file_id);
...
function drop_image(id){
    if (confirm('<?php echo $lang['store_goods_index_goods_del_confirm']; ?>')){
            var url = SITE_URL + '/index.php?con=member_flea&fun=drop_image';
...
function insert_img(id,item_url){
  var url = SITE_URL + '/index.php?con=member_flea&fun=insert_image';
  var j = 0;
  var y = 'sign';
  for (var i = 0; i < 5; i++){
  ...
  if(j==5){
    ...
    $.getJSON(url, {'id':id,'item_id':item_id}, function(data){
          ...
          $('#thumbnail_'+y).find('input[name="goods_file_id[]"]').val(data.file_id);
            ...
            $('#iframe_'+y).attr('src','index.php?con=member_flea&fun=image_upload&id='+y+'&item_id='+item_id+'&file_id='+data.file_id);
...
<?php if ($output['goods']['goods_id']=='') { ?>
        ...
        required : true,
                remote   : {
                    url  : 'index.php?con=member_flea&fun=check_class',
                    type : 'get',
                    data : {
                        cate_id : function(){
                            return $('#cate_id').val();
...
area_id: {
  required: true,
  remote   : {
        url  : 'index.php?con=index&fun=flea_area&check=1',
        type : 'get',
        data : {
            region_id : function(){
                return $('#area_id').val();
