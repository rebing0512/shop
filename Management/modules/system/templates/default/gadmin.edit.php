...
<div class="page">
    <div class="fixed-bar">
        <div class="item-title"><a class="back" href="index.php?con=admin&fun=gadmin" title="返回列表"><i
                        class="fa fa-arrow-circle-o-left"></i></a>
            <div class="subject">
                <h3><?php echo $lang['nc_limit_manage']; ?> - <?php echo $lang['nc_edit']; ?>
                    权限组“<?php echo $output['ginfo']['gname']; ?>”</h3>
    ...
    <form id="add_form" method="post" name="adminForm" style="margin-bottom: 50px;">
        ...
        <div class="ncap-form-default">
            <dl class="row">
                ...
                <dd class="opt">
                    <input type="text" id="gname" value="<?php echo $output['ginfo']['gname']; ?>" maxlength="40"
                           name="gname" class="input-txt">
                    <p class="notic">
                        如权限组名不变请忽略该选项。
                    </p>
                </dd>
        ...
        <div class="ncap-form-all">
          ...
          <span><input class="checkbox" type="checkbox" nctype="modulesAll">
                            ...
                            <div class="ncap-account-container">
                                ...
                                <ul class="ncap-account-container-list">
                                    <?php foreach ($val['child'] as $k => $v) { ?>
                                        <?php if ($key == 'system' && $k == 'admin') {
                                            continue;
                                        } ?>
                                        <li>
                                            <input class="checkbox" type="checkbox" value="<?php echo $k; ?>"
                                                   name="permission[<?php echo $key ?>][]" data-asd="<?=$output['ginfo']['limits'][$key][7]?>" <?php if (in_array($k, $output['ginfo']['limits'][$key])) {
                                                echo 'checked';
                                            } ?>>
                                            <?php echo $v; ?></li>
                                    <?php } ?>
            ...
            <?php } ?>
            <div class="fix-bot"><a href="JavaScript:void(0);" class="ncap-btn-big ncap-btn-green"
                                    onclick="document.adminForm.submit()"><?php echo $lang['nc_submit']; ?></a></div>
        </div>
    </form>
</div>
<script>
        ...
        $("#add_form").validate({
            ...
            rules: {
                gname: {
                    required: true,
                    remote: {
                        url: 'index.php?con=admin&fun=ajax&branch=check_gadmin_name&gid=<?php echo $output['ginfo']['gid']?>',
                        type: 'get',
                        data: {
                            gname: function () {
                                return $('#gname').val();
