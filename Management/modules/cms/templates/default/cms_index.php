...
$(document).ready(function(){
    ...
    $('#template_module_list').on('click', '[nctype="btn_add_page_module"]', function() {
        ...
        $.post("index.php?con=cms_index&fun=add_page_module", { module_title: page_module.module_title, module_name: page_module.module_name, module_type: page_module.module_type }, function(data) {
            if(data.result) {
    ...
    $('#template_module_list').on('click', '[nctype="btn_drop_module"]', function() {
        ...
        if(confirm('确认删除?')) {
            ...
            $.post("index.php?con=cms_index&fun=drop_module", { module_id: module_id }, function(data) {
                if(data.result) {
    ...
    $('#page_view').on('click', "[nctype='btn_show_page_module']", function() {
        ...
        $.post("index.php?con=cms_index&fun=update_page_module_show", { module_id: module_id }, function(data) {
            if(data.result) {
    ...
    $('#page_view').on('click', "[nctype='btn_hide_page_module']", function() {
        var $btn = $(this);
        var module_id = $btn.attr('data-module-id');
        $.post("index.php?con=cms_index&fun=update_page_module_hide", { module_id: module_id }, function(data) {
            if(data.result) {
                var $page_module = $btn.parents('.page-module');
    ...
    $('#page_view').on('click', "[nctype='btn_drop_page_module']", function() {
        if(confirm('确认删除')) {
            ...
            $.post("index.php?con=cms_index&fun=drop_page_module", { module_id: module_id }, function(data) {
                if(data.result) {
    ...
    $("#page_view").sortable({
        update: function(event, ui) {
            ...
            $.post("index.php?con=cms_index&fun=update_page_module_index", {page_module_id_string: page_module_id_string}, function(data) {
                if(!data.result) {
                    showError(data.message);
                }
            }, 'json');
    ...
    $("#btn_index_build").click(function() {
        $.getJSON('index.php?con=cms_index&fun=cms_index_build', function(data) {
            if(data.result) {
    ...
    $('#btn_add_module').click(function() {
        ...
        if($module_frame_list.html() === '') {
            $.getJSON('index.php?con=cms_index&fun=get_module_frame_list', function(data) {
                if(data.frame_list) {
                    var template_data = {};
    ...
    $('#btn_save_module').click(function() {
        ...
        $.post("index.php?con=cms_index&fun=save_module", {module_title: module_title, frame_name: frame_name, frame_block: frame_block, module_display_title_state: module_display_title_state}, function(data) {
            if(data.result) {
    ...
    $('#page_view').on('click', "[nctype='btn_module_style_select']", function() {
        ...
        $.post("index.php?con=cms_index&fun=update_page_module_style", {module_id: module_id, module_style: module_style}, function(data) {
            if(data.result) {
                var $page_module = $btn.parents('.page-module');
    ...
    $('#page_view').on('click', "[nctype='btn_module_view_change']", function() {
        ...
        $.post("index.php?con=cms_index&fun=update_page_module_view", {module_id: module_id, module_view: module_view}, function(data) {
            if(!data.result) {
                showError(data.message);
            }
        }, 'json');
...
<div class="page">
  ...
  <div class="fix-bot"><a class="ncap-btn-big ncap-btn-orange" href="index.php?con=cms_index&fun=cms_index_preview" target="_blank">页面预览</a> <a class="ncap-btn-big ncap-btn-blue ml10" id="btn_index_build" href="Javascript: void(0)"><?php echo $lang['cms_index_build'];?></a> <a class="ncap-btn-big ncap-btn-green ml10" href="<?php echo CMS_SITE_URL;?>" target="_blank"><?php echo $lang['cms_index_go'];?></a></div>
...
<div id="page_module_<%=module_id%>" data-module-id="<%=module_id%>" data-module-name="<%=module_name%>" class="page-module <%=module_css%> <%=module_state_css%> module-css-<%=module_type%> module-style-<%=module_style%>">
    ...
    <a href="index.php?con=cms_index&fun=module_edit&module_id=<%=module_id%>" class="edit" title="<?php echo $lang['nc_edit'];?>"></a>
