...
class store_listControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		if (intval($_GET['cate_id']) > 0){
			$nav_link[1]['link'] = 'index.php?con=shop_search';
			...
			if ($nav['sc_parent_id'] > 0){
				...
				$nav_link[] = array(
					'title'=>$tmp['sc_name'],
					'link'=>"index.php?con=store_list&cate_id=".$nav['sc_parent_id']
				);
			}
			//存入当前级
			$nav_link[] = array(
		...
		Tpl::output('purl', urlShop($purl['con'], $purl['fun'], $purl['param']));
	...
	function getParam() {
	    ...
	    unset($param['con']);
	    $purl['fun'] = $param['fun'];
	    unset($param['fun']); unset($param['curpage']);
	    $purl['param'] = $param;
	    return $purl;
	}
}
