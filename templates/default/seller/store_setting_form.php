...
<div class="ncsc-form-default">
  <form method="post"  action="index.php?con=store_setting&fun=store_setting" enctype="multipart/form-data" id="my_store_form">
    ...
    <dl>
      ...
      <dd>
          <textarea name="store_zy" rows="2" class="textarea w400"  maxlength="140" ><?php echo $output['store_info']['store_zy'];?></textarea>
        <p class="hint">关键字最多可输入140字，请用","进行分隔，例如”男装,女装,童装”</p>
      </dd>
    </dl>
    <dl>
    ...
    <dl>
      ...
      <dl class="setup store-logo">
        ...
        <dd>
          <p>
          <img src="<?php if(empty($output['store_info']['store_avatar'])){ echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.$output['setting_config']['default_store_avatar'];}else{ echo getStoreLogo($output['store_info']['store_avatar']);}?>" nc_type="store_avatar" />
          </p>
          <p>
      ...
      <?php if (!empty($output['store_info']['recommend'])):?>
          <dl class="setup store-recommend">
              <dt>店铺推荐图片：</dt>
              <dd>
                  <p>
                      <img src="<?php if(empty($output['store_info']['recommend_pic'])){ echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.$output['setting_config']['default_store_avatar'];}else{ echo getStoreLogo($output['store_info']['recommend_pic']);}?>" nc_type="store_recommend" />
                  </p>
                  <p>
                      <input type="hidden" value="" name="recommend_pic" id="recommend_pic">
                      <input name="store_recommend_pic" type="file"  hidefocus="true" id="store_recommend_pic"/>
                  </p>
                  <p class="hint">此处为首页好物推荐店铺显示图片</p>
              </dd>
          </dl>
      <?php endif;?>
    ...
    <dl>
      ...
      <dl>
          <dt>店铺简介链接<?php echo $lang['nc_colon'];?></dt>
          <dd>
              <input class="text w200" name="intro" maxlength="200" type="text"  id="intro" value="<?php echo $output['store_info']['intro'];?>" />
          </dd>
      </dl>
      <dl>
          <dt>名师箴言<?php echo $lang['nc_colon'];?></dt>
          <dd>
              <input class="text w200" name="talk" maxlength="50" type="text"  id="talk" value="<?php echo $output['store_info']['talk'];?>" style="width: 500px;" />
          </dd>
      </dl>
    <dl>
...
function call_back(picname){
    //console.log(picname)
    picname = JSON.parse(picname);
    if (picname.id == '_pic'){
        $('#store_avatar').val(picname.basename);
        $('img[nc_type="store_avatar"]').attr('src','<?php echo UPLOAD_SITE_URL.'/'.ATTACH_STORE;?>/'+picname.basename);
        $('#'+picname.id).val('');
    } else if (picname.id = 'store_recommend_pic'){
        $('#recommend_pic').val(picname.basename);
        $('img[nc_type="store_recommend"]').attr('src','<?php echo UPLOAD_SITE_URL.'/'.ATTACH_STORE;?>/'+picname.basename);
        $('#'+picname.id).val('');
    }
...
$(function(){
	...
	$('#store_recommend_pic').change(uploadChange);
	...
	function ajaxFileUpload(id)
	{
		...
		(
			{
				url:'index.php?con=cut&fun=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_STORE;?>&id='+id,
				secureuri:false,
				fileElementId:id,
				dataType: 'json',
				success: function (data, status)
				{
				    if (id == 'store_recommend_pic'){
				        var ratio = 0;
                    } else {
				        var ratio = 1;
                    }
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','index.php?con=cut&fun=pic_cut&x=800&y=800&resize=1&ratio='+ratio+'&url='+data.url+'&id='+id,680);
					}else{
						alert(data.msg);$('#'+id).bind('change',uploadChange);
					}
				},
				error: function (data, status, e)
				{
					alert('上传失败');$('#'+id).bind('change',uploadChange);
				}
			}
		)
	};
