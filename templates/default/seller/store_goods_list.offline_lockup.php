...
<form method="get" action="index.php">
  <table class="search-form">
    <input type="hidden" name="con" value="store_goods_offline" />
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
        ...
        <?php } else {?>
        <span class="tip" title="该商品参加特卖活动期间不能进行编辑及删除等操作,可以编辑赠品和推荐组合"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlShop('store_goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-darkgray"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
