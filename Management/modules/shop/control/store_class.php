...
class store_classControl extends SystemControl
{
    ...
    public function store_classOp()
    {
        ...
        $core_categorys = $systemcategoryModel::getSystemCategory()['result']['category'];

        $type = [];

        foreach ($core_categorys as $item){
        ...
        foreach ($store_class_list as $key=>$value){
            $store_class_list[$key]['core_category_name'] = $type[$value['core_category_id']];
        }
    ...
    public function store_class_addOp()
    {
        ...
        foreach ($core_categorys as $item)
        {
            $core_categories[$item['id']] = $item['name'];
        }
        if (chksubmit()) {
            //验证
            ...
            $obj_validate->validateparam = array(
                ...
                array('input'=> $_POST['core_category_id'] ,'require'=>'true','message' => '请选择店铺分类所属系统核心分类'),
            );
            $error = $obj_validate->validate();
            if ($error != '') {
                showMessage($error);
            } elseif (!in_array($_POST['core_category_id'],array_keys($core_categories))) {
                showMessage('所选的系统核心分类不存在');
            } else {
                $insert_array = array();
                $insert_array['sc_name'] = $_POST['sc_name'];
                $insert_array['sc_bail'] = intval($_POST['sc_bail']);
                $insert_array['sc_sort'] = intval($_POST['sc_sort']);
                $insert_array['core_category_id'] = intval($_POST['core_category_id']);
                $result = $model_class->addStoreClass($insert_array);
                if ($result) {
                    $url = array(
                        array(
                            'url' => 'index.php?con=store_class&fun=store_class_add',
                            'msg' => $lang['continue_add_store_class'],
                        ),
                        array(
                            'url' => 'index.php?con=store_class&fun=store_class',
        ...
        Tpl::setDirquna('shop');
        Tpl::output('core_categories',$core_categories);
    ...
    public function store_class_editOp()
    {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                $update_array['core_category_id'] = intval($_POST['core_category_id']);
                ...
                if ($result) {
                    ...
                    showMessage($lang['nc_common_save_succ'], 'index.php?con=store_class&fun=store_class');
                } else {
                    showMessage($lang['nc_common_save_fail']);
                }
            }
    ...
    public function store_class_delOp()
    {
        ...
        showMessage($lang['nc_common_del_fail'], 'index.php?con=store_class&fun=store_class');
    }
