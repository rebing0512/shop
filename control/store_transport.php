...
class store_transportControl extends BaseSellerControl {
    ...
    public function saveOp(){
        ...
        if ($result){
            header('location: index.php?con=store_transport&type='.$_POST['type']);exit;
        }else{
            showMessage(Language::get('transport_op_fail'),$_SERVER['HTTP_REFERER'],'html','error');
        }
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'offpay_area':
                $menu_array = array(
                1=>array('menu_key'=>'transport',   'menu_name'=>Language::get('nc_member_path_postage'),       'menu_url'=>'index.php?con=store_transport'),
                );
                if (checkPlatformStore()) {
                    $menu_array[] = array(
                        'menu_key' => 'offpay_area',
                        'menu_name' => '货到付款地区',
                        'menu_url' => 'index.php?con=store_transport&fun=offpay_area'
                    );
                }
                break;
        }
