...
$(function(){
		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?con=common&fun=pic_cut&type=circle&x=120&y=120&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
	...
	$('#mastername').focus(function(){
		ajax_form('choose_master', '<?php echo $lang['circle_choose_master'];?>', 'index.php?con=circle_manage&fun=choose_master', 640);
	});
	...
	$('#circle_form').validate({
        ...
        rules : {
        	c_name : {
        		...
        		maxlength : 12,
            	remote : {
            		url : 'index.php?con=circle_manage&fun=check_circle_name',
                    type: 'get',
                    data:{
                    	name : function(){
                            return $('#c_name').val();
            ...
            mastername : {
            	required : true,
                remote   : {
                    url :'index.php?con=circle_manage&fun=check_member&branch=ok',
                    type:'get',
                    data:{
                        name : function(){
