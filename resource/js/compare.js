...
function loadCompare(isrefresh){
	...
	if(isrefresh == true){
		$("#comparelist").load('index.php?con=compare&fun=showcompare');
	}
}
//添加对比商品
function addCompare(gid){
	...
	if (gid > 0){
       	$.ajax({
            type:"GET",
            dataType: "json",
            url:'index.php?con=compare&fun=addcompare&id='+gid,  
            async:false,
...
function delCompare(gid,type){	
	$.ajax({
        type:"GET",
        dataType: "json",
        url:'index.php?con=compare&fun=delcompare&gid='+gid+'&type='+type,  
        async:false,
        success: function(data){  
            if(data.done == true) {
            	//将对比按钮置为未对比状态
            	...
            	if (type == 'mini'){
                    ...
                    go('index.php?con=compare&gids='+data.gid_str);
                }
            }
            $("#lockcompare").val('unlock');//解除加入对比按钮的锁定
        }
...
function initCompare(){
	...
	//根据是否已加入对比，显示不同样式
	$.getJSON('index.php?con=compare&fun=checkcompare', function(data){
        if(data){
