...
class RSAUtils {
	...
	public static function encryptByPrivateKey($data) {
		...
		$pem =file_get_contents(SHOP_SITE_URL.'/api/payment/jdpay/key/seller_rsa_private_key.pem');
	
		$pi_key =  openssl_pkey_get_private($pem);//这个函数可用来判断私钥是否是可用的，可用返回资源id Resource id
		$encrypted="";
	...
	public static function decryptByPublicKey($data) {
		...
		$pem =file_get_contents(SHOP_SITE_URL.'/api/payment/jdpay/key/wy_rsa_public_key.pem');
		$pu_key =  openssl_pkey_get_public($pem);//这个函数可用来判断公钥是否是可用的，可用返回资源id Resource id
		$decrypted = "";
