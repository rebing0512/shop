...
class tenpay{
	...
	private $gateway   = 'https://wap.tenpay.com/cgi-bin/wappayv2.0/wappay_init.cgi';
	/**
	 * 支付接口标识
	 *
	 * @var string
	...
	public function submit($param){
		...
		/* 商户号 */
		$bargainor_id = $param['tenpay_partnerid'];//"1900000109";
		
		/* 密钥 */
		$key = $param['tenpay_partnerkey'];//"8934e7d15453e97507ef794cf7b0519d";
		
		/* 返回处理地址 */
		$return_url = MOBILE_SITE_URL."/api/payment/tenpay/notify_url.php";
		...
		$sp_billno = $param['order_sn'].'_'.$param['order_type'];
		
		/* 财付通交易单号，规则为：10位商户号+8位时间（YYYYmmdd)+10位流水号 */
		$transaction_id = $bargainor_id . $strDate . $strReq;
		
		/* 商品价格（包含运费），以分为单位 */
		$total_fee = floatval($param['order_amount'])*100;
		...
		$reqHandler->setParameter("desc", $param['order_sn']);	        //商品名称
        $reqHandler->setParameter("attach", $param['order_type']);	//自定义参数
