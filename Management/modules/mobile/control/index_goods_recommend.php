...
class index_goods_recommendControl extends SystemControl{
    ...
    public function indexOp(){
        $model_rec = Model('index_recommend_goods');

        Tpl::setDirquna('mobile');
        Tpl::showpage('index_goods_recommend.index');
    }
    ...
    public function addOp(){
        ...
        $goods_list = array();
        if ($rec_gc_id > 0) {
            $rec_list = Model('index_recommend_goods')->getGoodsRecommendList(array('rec_gc_id'=>$rec_gc_id),'','','*','','rec_goods_id');
            if (!empty($rec_list)) {
                ...
                if (!empty($goods_list)) {
                    foreach ($goods_list as $k => $v) {
                        $goods_list[$k]['goods_image'] = cthumb($v['goods_image'],60,$v['store_id']);
                        foreach ($rec_list as $item){
                            if ($item['rec_goods_id']==$goods_list[$k]['goods_id']){
                                $goods_list[$k]['sort'] = $item['sort'];
                            }
                        }
                    }
                }
        ...
        foreach ($list as $k=>$item){
            foreach ($category['result']['category'] as $key=>$value){
                if ($item['h_type']==$value['id']){
                    $list[$k]['name'] = $value['name'].'>'.$item['name'];
                }
            }
        }

        Tpl::output('list',$list);
        Tpl::output('rec_list',json_encode($rec_list));
        Tpl::output('goods_list_json',json_encode($goods_list));
        Tpl::output('goods_list', $goods_list);
        Tpl::output('rec_info', is_array($rec_list) ? current($rec_list) : array());

        Tpl::setDirquna('mobile');
        Tpl::showpage('index_goods_recommend.edit');
    }
    ...
    public function saveOp(){
        ...
        if (is_array($_POST['goods_id_list'])) {
            foreach ($_POST['goods_id_list'] as $k => $goods_id) {
                ...
                $data[$k]['sort'] = $_POST['sort'][$k];
            }
        }
        $insert = $model_rec->addGoodsRecommend($data);
        if ($insert) {
            showMessage('保存成功','index.php?con=index_goods_recommend&fun=index');
        }
    }
    ...
    public function get_xmlOp(){
		...
		//商品数量计算
		$model_rec = Model('index_recommend_goods');
		$rec_list = $model_rec->field('count(*) as rec_count,rec_gc_id')->group('rec_gc_id')->select();;
		//var_dump($rec_list);
		$count_arr = array();
		foreach($rec_list as $v){
		   $count_arr[$v['rec_gc_id']] = $v['rec_count'];
		}
		
		$order = "h_type asc,sort desc";	
        $list =$model->table('recommend')->where($condition)->order($order)->page($_POST['rp']?:15)->select();
        $data = array();
        ...
        foreach ($list as $k => $v) {
            $list = array();
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?con=index_goods_recommend&fun=add&rec_gc_id={$v['id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $list['rec_gc_name'] = $this->my_category[$v['h_type']].">".$v['name'];
            $list['rec_count'] =  isset($count_arr[$v['id']])?$count_arr[$v['id']]:0;//0;//$v['rec_count'];
        ...
        Tpl::output('category',$category['result']['category']);
        //var_dump($rec_list);
        $recommend = Model('recommend')->select();
        foreach ($rec_list as $k=>$v){
            foreach ($recommend as $key=>$value){
                if ($v['rec_gc_id']==$value['id']){
                    foreach ($category['result']['category'] as $kk=>$vv){
                        if ($vv['id']==$value['h_type']){
                            $rec_list[$k]['h_type'] = $vv['name'];
                        }
                    }
                }
            }
        ...
        foreach ($rec_list as $v) {
            $list = array();
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['rec_gc_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?con=index_goods_recommend&fun=add&rec_gc_id={$v['rec_gc_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $list['rec_gc_name'] = $v['rec_gc_name'];
            $list['rec_count'] = $v['rec_count'];
            $list['h_type'] = $v['h_type'];
    ...
    public function get_goods_listOp(){
        ...
        $recommend = Model('recommend')->where('id = '.$gc_id)->find();
        $model_goods = Model('goods');
        $condition = array();
        //$condition['category_id'] = $recommend['h_type'];
        ...
        foreach($goods_list as $v) {
            ...
            $img = cthumb($v['goods_image'],60,$v['store_id']);
            $html .= <<<EOB
            <li>
