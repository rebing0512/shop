...
function update_flex(){
	$('.ncap-stat-general-single').load('index.php?con=stat_trade&fun=get_plat_sale&'+$("#formSearch").serialize(),
		function(){
			$('.timer').each(count);
     	});
    $("#flexigrid").flexigrid({
        url: 'index.php?con=stat_trade&fun=get_sale_xml&'+$("#formSearch").serialize(),
        colModel : [
...
function getStatdata(type){
	$('#'+type+'_div').load('index.php?con=stat_trade&fun=sale_trend&type='+type+'&'+$("#formSearch").serialize());
}
function fg_operation(name, bDiv){
    var stat_url = 'index.php?con=stat_trade&fun=sale';
