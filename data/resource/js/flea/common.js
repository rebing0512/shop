...
function login_dialog(){
	CUR_DIALOG = ajax_form('login','登录','index.php?con=login&inajax=1',360,1)
}
...
function collect_store(fav_id,jstype,jsobj){
	$.get('index.php?con=index&fun=login', function(result){
	    if(result=='0'){
	    	...
	    	var url = 'index.php?con=member_favorites&fun=favoritesstore';
...
function collect_goods(fav_id,jstype,jsobj){
	$.get('index.php?con=index&fun=login', function(result){
	    if(result=='0'){
	    	...
	    	var url = 'index.php?con=member_favorites&fun=favoritesgoods';
	    	$.getJSON(url, {'fid':fav_id}, function(data){
	            if (data.done)
	            {
	                showDialog(data.msg, 'succ','','','','','','','','',2);
...
//动态加载js，css
//$.include(['http://www.xxx.com/script/a.js','/css/css.css']);
$.extend({
