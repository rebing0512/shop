...
<div class="page">
  ...
  <div class="flex-table-search">
    <form method="get" name="formSearch">
      <input type="hidden" name="con" value="circle_class">
      <input type="hidden" name="fun" value="class_list">
      <div class="sDiv">
    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr class="edit"  data-id="<?php echo $v['class_id'];?>">
          ...
          <td class="handle"><a href="javascript:void(0);" onclick="fg_del(<?php echo $v['class_id'];?>)" class="btn red"><i class="fa fa-trash-o"></i><?php echo $lang['nc_del'];?></a><a href="index.php?con=circle_class&fun=class_edit&classid=<?php echo $v['class_id'];?>" class="btn blue"><i class="fa fa-pencil-square-o"></i><?php echo $lang['nc_edit'];?></a></td>
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?con=circle_class&fun=class_add';
    } else if (name == 'del') {
...
function fg_del(ids) {
    ...
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?con=circle_class&fun=class_del', {id:id}, function(data){
            if (data.state) {
                location.reload();
            } else {
                showError(data.msg)
