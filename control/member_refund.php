...
class member_refundControl extends BaseMemberControl {
    ...
    public function add_refundOp(){
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?con=member_order&fun=index','error');
        }
        $condition = array();
        ...
        if (chksubmit() && $goods_id > 0){
            ...
            $show_url = 'index.php?con=member_return&fun=index';
            ...
            if ($refund_array['refund_type'] != '2') {
                ...
                $show_url = 'index.php?con=member_refund&fun=index';
            }
    ...
    public function add_refund_allOp(){
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                $model_refund->editOrderLock($order_id);
                showDialog(Language::get('nc_common_save_succ'),'index.php?con=member_refund&fun=index','succ');
            } else {
                showDialog(Language::get('nc_common_save_fail'),'reload','error');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member_order':
                $menu_array = array(
                array('menu_key'=>'buyer_refund','menu_name'=>Language::get('nc_member_path_buyer_refund'), 'menu_url'=>'index.php?con=member_refund'),
                array('menu_key'=>'buyer_return','menu_name'=>Language::get('nc_member_path_buyer_return'), 'menu_url'=>'index.php?con=member_return'),
                array('menu_key'=>'buyer_vr_refund','menu_name'=>'虚拟兑码退款',    'menu_url'=>'index.php?con=member_vr_refund'));
                break;
        }
