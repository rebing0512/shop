...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=circle_member&fun=get_xml',
        colModel : [
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?con=circle_member&fun=member_del', {id:id}, function(data){
            if (data.state) {
...
function fg_recommend(id, value) {
    $.getJSON('index.php?con=circle_member&fun=member_recommend', {id:id, value:value}, function(data){
        if (data.state) {
            $("#flexigrid").flexReload();
        } else {
            showError(data.msg)
