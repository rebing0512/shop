  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        ...
        <li><a href="index.php?con=predeposit&fun=pd_cash_list"><?php echo $lang['admin_predeposit_cashmanage']; ?></a></li>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=predeposit&fun=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=predeposit&fun=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: 'index.php?con=predeposit&fun=get_xml',
        colModel : [
...
function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?con=predeposit&fun=recharge_del', {id:id}, function(data){
            if (data.state) {
