...
function thumb($goods = array(), $type = ''){
    ...
    if (empty($goods)){
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type)."?ref=t400";
    }
    if (array_key_exists('apic_cover', $goods)) {
        $goods['goods_image'] = $goods['apic_cover'];
    }
    if (empty($goods['goods_image'])) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type)."?ref=e400";
    }
    ...
    if (!file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_GOODS.'/'.$store_id.'/'.$file)){
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type)."?ref=t404";
    }
...
function cthumb($file, $type = '', $store_id = false) {
    ...
    if (empty($file)) {
        return UPLOAD_SITE_URL . '/' . defaultGoodsImage ( $type ).'?ref=400';
    }
    $search_array = explode(',', GOODS_IMAGES_EXT);
    ...
    if ( !file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/' . $store_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) )) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type).'?ref=404&a='.BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/' . $store_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file));
    }
    $thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_GOODS;
 ...
 * @return string
 */
function athumb($file, $type = '', $store_id = false) {
    ...
    $search_array = explode(',', ATTACH_IMAGES_EXT);
    $file = str_ireplace($search_array,'',$file);
    $fname = basename($file);
    // 取店铺ID
    if ($store_id === false || !is_numeric($store_id)) {
    ...
    // 本地存储时，增加判断文件是否存在，用默认图代替
    if ( !file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_ARTICLE_ALBUM . '/' . $store_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) )) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
    }
    $thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_ARTICLE_ALBUM;
    return $thumb_host . '/' . $store_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file));
}

/**
 * 取得相册列表缩略图的完整URL路径，接收商品信息数组，返回所需的商品缩略图的完整URL
 *
 * @param array $goods 相册信息数组
 * @param string $type 缩略图类型  值为60,240,360,1280
 * @return string
 */
function sthumb($thumb = array(), $type = ''){
    $type_array = explode(',_', ltrim(GOODS_IMAGES_EXT, '_'));
    if (!in_array($type, $type_array)) {
        $type = '240';
    }
    ...
    if (array_key_exists('apic_cover', $thumb)) {
        $thumb['goods_image'] = $thumb['apic_cover'];
    }
    if (empty($thumb['goods_image'])) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
    }
    $search_array = explode(',', ATTACH_IMAGES_EXT);
    $file = str_ireplace($search_array,'',$thumb['goods_image']);
    $fname = basename($file);
    //取店铺ID
    if (preg_match('/^(\d+_)/',$fname)){
        $store_id = substr($fname,0,strpos($fname,'_'));
    }else{
        $store_id = $thumb['store_id'];
    }
    $file = $type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file);
    if (!file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_ARTICLE_ALBUM.'/'.$store_id.'/'.$file)){
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
    }
    $thumb_host = UPLOAD_SITE_URL.'/'.ATTACH_ARTICLE_ALBUM;
    return $thumb_host.'/'.$store_id.'/'.$file;
 ...
 * @return string
 */
function rzThumb($image_name = '', $type = ''){
    if (!in_array($type, array('240','1024'))) $type = '240';
    if (empty($image_name)){
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage('240');
    }
    ...
    $file_path = ATTACH_STORE_JOININ.DS.str_ireplace('.', '_'.$type.'.', $image_name);
    if(!file_exists(BASE_UPLOAD_PATH.DS.$file_path)) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage('240');
    }
    return UPLOAD_SITE_URL.DS.$file_path;
 ...
 * @return string
 */
function brandbigImage($image_name = '') {
    if ($image_name != '') {
        return UPLOAD_SITE_URL.'/'.ATTACH_BRAND_BIG.'/'.$image_name;
    }
    return UPLOAD_SITE_URL.'/'.ATTACH_COMMON.'/default_brand_image.gif';
}
...
function orderPaymentName($payment_code) {
    return str_replace(
            array('offline','online','alipay','tenpay','jdpay','predeposit','wxpay','wx_saoma','wx_jsapi'),
            array('货到付款','在线付款','支付宝','财付通','网银在线','站内余额支付','微信支付','微信扫码','微信支付'),
            $payment_code);
}
...
defined("TTShop") || exit("Access Invalid!");
