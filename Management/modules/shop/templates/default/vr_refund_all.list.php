...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=vr_refund&fun=get_all_xml&waiting=<?php echo $_GET['waiting'];?>&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=vr_refund&fun=get_all_xml&waiting=<?php echo $_GET['waiting'];?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?con=vr_refund&fun=get_all_xml&waiting=<?php echo $_GET['waiting'];?>',
        colModel : [
