...
<?php echo $output['decoration_nav']['style'];?>
  ...
  <div class="ncsl-nav">
    ...
    <div id="nav" class="ncs-nav">
      <ul>
       <li class="<?php if($output['page'] == 'index'){?>active<?php }else{?>normal<?php }?>"><a href="<?php echo urlShop('show_store', 'index', array('store_id'=>$output['store_info']['store_id']));?>"><span>首页<i></i></span></a></li>
        <li id="store_nav_class_button" class="normal ncs-nav-classes"> 
          <!-- 店铺商品分类 --> 
          <a href="javascript:;"><span>商品分类<i></i></span></a>
        ...
        <li class="<?php if($output['page'] == 'mall'){?>active<?php }else{?>normal<?php }?>"><a href="<?php echo urlShop('show_store', 'mall', array('store_id'=>$output['store_info']['store_id']));?>"><span><?php echo $lang['nc_store_index'];?><i></i></span></a></li>
...
<?php include template('store/footer');?>
...
$(function(){
	...
	$('.favorites').mouseover(function(){
		var $this = $(this);
		if(storeTrends){
			$.getJSON('index.php?con=show_store&fun=ajax_store_trend_count&store_id=<?php echo $output['store_info']['store_id'];?>', function(data){
	...
	$('a[nctype="share_store"]').click(function(){
		...
		login_dialog();
		<?php } else {?>
		ajax_form('sharestore', '分享店铺', 'index.php?con=member_snsindex&fun=sharestore_one&inajax=1&sid=<?php echo $output['store_info']['store_id'];?>');
		<?php }?>
	});
