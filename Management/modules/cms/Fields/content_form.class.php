...
class content_form {
    ...
    public function __construct($modelid, $catid='') {
        $model = Model('ncms_model')->select();
        $this->model = array();
        foreach ($model  as &$value) {
            $this->model[$value['modelid']] = $value;
        }
        if ($modelid) {
    ...
    public function setModelid($modelid, $catid) {
        ...
        $this->catid = $catid;
        $fields =Model('ncms_model_field')->where(array('modelid'=>$this->modelid,'disabled'=>0))->order('listorder asc')->select();
        $this->fields = array();
        $datas = array();
        foreach ($fields as &$value) {
            $datas[$value['field']] = $value;
            $this->fields=  $datas;
        }
...
function editor($field, $value, $fieldinfo) {
    ...
    $width  = $setting['width'];
    $upload_state =  $setting['upload_state'];
    $media_open =  $setting['media_open'];
    $type =  $setting['type'];
    ...
    if (empty($setting['width'])) {
      ...
      // public static function editor($id, $value='', $width='700px', $height='300px', $style='visibility:hidden;',$upload_state="true", $media_open=false, $type='all') {

    $form = Form::editor($field,$value,$width,$height,'',$upload_state,$media_open,$type);
    //javascript
    $this->formJavascript .= "
            //增加编辑器验证规则
            jQuery.validator.addMethod('editor{$field}',function(){

                return " . ($fieldinfo['minlength'] ? "$('#{$field}').val();" : "true") . "
            });
    ";
    //错误提示
...
function catid($field, $value, $fieldinfo) {
    ...
    // if (ACTION_NAME == 'add' && defined("IN_ADMIN") && IN_ADMIN) {
        // $publish_str = "<a href='javascript:;' onclick=\"omnipotent('selectid','Content/Content/public_othors','同时发布到其他栏目',1);return false;\" style='color:#B5BFBB'>[同时发布到其他栏目]</a>
...
function title($field, $value, $fieldinfo) {
    ...
    $style_color = explode(':', $style_arr[0]);
    //是否粗体
    $style_font_weight =explode(':', $style_arr[1]);
    ...
    $str = '<link href="'.RESOURCE_SITE_URL.'/js/colorpicker/evol.colorpicker.css" rel="stylesheet" type="text/css">';
    $str.= '<script src="'.RESOURCE_SITE_URL.'/js/colorpicker/evol.colorpicker.min.js"></script>';

    $str.= '<input type="text" style="width:400px;' . $style . '" name="info[' . $field . ']" id="' . $field . '" value="' .forTag($value) . '" style="' . $style . '" class="input input_hd J_title_color" placeholder="请输入标题" onkeyup="strlen_verify(this, \'' . $field . '_len\', ' . $fieldinfo['maxlength'] . ')" />
                <input type="hidden" name="style_font_weight" id="style_font_weight" value="' . $style_font_weight[1] . '">';
    //后台的情况下
    if (APP_ID ==='ncms_adminmin')
    $str .= '<input type="button" class="btn" id="check_title_alt" value="标题检测" onclick="$.get(\'' .'index.php?con=ncms_model_field&fun=public_check_title'. '\', {data:$(\'#title\').val()}, function(data){if(data.status==false) {$(\'#check_title_alt\').val(\'标题重复\');$(\'#check_title_alt\').css(\'background-color\',\'#FFCC66\');} else if(data.status==true) {$(\'#check_title_alt\').val(\'标题不重复\');$(\'#check_title_alt\').css(\'background-color\',\'#F8FFE1\')}},\'json\')" style="width:73px;"/><div class="small_box" style="width:80px;height:60px;display:inline-block;"><input id="style_color" type="hidden" name="style_color" value="' . $style_color[1] . '"><img src="'.ADMIN_TEMPLATES_URL.'/images/bold.png" width="10" height="10" onclick="input_font_bold()" style="cursor:hand;padding:0 5px;"/></div>';
    $str .= ' <span>还可输入<B><span id="title_len">' . $fieldinfo['maxlength'] . '</span></B> 个字符</span>';
    $str .='<script>$(function(){ $("#style_color").colorpicker($("#style_color").on("change.color", function(event, color){$("#title").css( {"color":color });}));})</script>';
    return $str;
}
...
function image($field, $value, $fieldinfo) {
  ...
  //错误提示
    ...
    $setting = unserialize($fieldinfo['setting']);
      $str = "<div class='input-file-show'><span class='type-file-box'>";
      $str.= "<input type='text'  class='type-file-text textfield' />";
      $str.= "<input type='button' name='button' id='button' value='选择上传...' class='type-file-button' />";
      $str.= "<input id='{$field}' name='{$field}' type='file' class='type-file-file' size='30' hidefocus='true' />";
       $str.= "<input  name='file[$name]' type='hidden' value='{$val}' />";
      $str.= "</span></div>";
      $str.= '<script>$(function(){$("#'.$field.'").change(function(){$(this).siblings(".textfield").val($('.$field.').val());});})</script>';
...
function datetime($field, $value, $fieldinfo) {
    ...
    return Form::date("info[{$field}]", $value, $format);
}
...
function author($field, $value, $fieldinfo) {
    ...
    return '<input type="text" class="input" name="info[' . $field . ']" value="' . forTag($value) . '" style="' . $width . '" placeholder="请输入' . $fieldinfo['name'] . '信息">';
}
...
function copyfrom($field, $value, $fieldinfo) {
    ...
    return "<input type='text' name='info[{$field}]' value='".forTag($value)."' style='width:{$width}px;' class='input' placeholder='信息来源'/>";
}
/**
 * 转向地址 字段类型表单组合处理
 * @param type $field 字段名
