...
class queueLogic {
    ...
    public function editGroupbuySaleCount($groupbuy_info) {
        ...
        if (!$update) {
            return callback(false,'更新特卖信息失败groupbuy_id:'.$groupbuy_info['groupbuy_id']);
        } else {
            return callback(true);
        }
    }
    ...
    public function editRptState($input_rpt_info, $pay_sn) {
        ...
        if ($update) {
            ...
            if ($update) {
                ...
                $param['rpacket_url'] = urlShop('member_redpacket', 'index');
                $send->send($param);
            } else {
    ...
    public function createGoodsQRCode($param) {
        ...
        foreach ($param['goodsid_array'] as $goods_id) {
            // 生成商品二维码
            $PhpQRCode->set('date',WAP_SITE_URL.DS.'index.php?con=goods&fun=detail&goods_id='.$goods_id);
