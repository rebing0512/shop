...
class pointcartControl extends BasePointShopControl {
    public function __construct() {
        ...
        if (C('pointprod_isuse') != 1){
             ...
             echo json_encode(array('done'=>false,'msg'=>L('pointcart_unavailable'),'url'=>'index.php')); die;
        }
        //验证是否登录
    ...
    public function step1Op(){
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?con=pointprod','error');
        }
    ...
    public function step2Op() {
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?con=pointcart','error');
        }
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?con=pointcart&fun=step1','error');
        }
        $order_id = $data['data']['order_id'];
        @header("Location:index.php?con=pointcart&fun=step3&order_id=".$order_id);
    }
    /**
     * 流程第三步
     */
