...
<div id="footer">
	...
	| <a  <?php if($nav['nav_new_open']){?>target="_blank" <?php }?>href="<?php switch($nav['nav_type']){
    	...
    	case '1':echo ncUrl(array('con'=>'search','nav_id'=>$nav['nav_id'],'cate_id'=>$nav['item_id']), '', 'www');break;
    	case '2':echo ncUrl(array('con'=>'article','nav_id'=>$nav['nav_id'],'ac_id'=>$nav['item_id']), '', 'www');break;
    	case '3':echo ncUrl(array('con'=>'activity','activity_id'=>$nav['item_id'],'nav_id'=>$nav['nav_id']), 'activity', 'www');break;
...
function load_cart_information(){
	$.getJSON('index.php?con=cart&fun=ajaxcart', function(result){
	    if(result){
	        var result  = result;
	       	...
	       	if(result.goods_all_num >0){
	            ...
	            for (i = 0; i < data.length; i++)
	            {
	            	...
	            	html+="<td class='name'><a href='<?php echo SiteUrl.'/';?>index.php?con=goods&goods_id="+data[i]['goodsid']+"' title='"+data[i]['gname']+"' target='_top'>"+data[i]['gname']+"</a></td>";
...
function drop_topcart_item(store_id, spec_id){
    ...
    $.getJSON('index.php?con=cart&fun=drop&specid=' + spec_id + '&storeid=' + store_id, function(result){
        if(result.done){
            //删除成功
...
$(function(){
	...
	$.include('index.php?con=scan&type=updown');
	<?php }?>
});
</script>
