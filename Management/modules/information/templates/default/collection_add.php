...
<div class="page">
    <div class="fixed-bar">
        <div class="item-title"><a class="back" href="index.php?con=mb_collection" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
            <div class="subject">
                <h3><?php echo $output['action'];?>圆图信息模式</h3>
                <h5><?php echo $output['action'];?>圆图信息</h5>
            </div>
        </div>
    </div>
    <!-- 操作说明 -->
        ...
        <div class="ncap-form-default">
            <dl class="row">
                ...
                <dd class="opt">
                    ...
                    <span class="err"></span>
                    <p class="notic">如果有称号请将名字和称号用"|"分隔，例如"名字|称号"</p>
                </dd>
            </dl>
            <dl class="row">
                <dt class="tit">
            ...
            <dl class="row">
                <dt class="tit">
                    <label for="level"><em>*</em>级别</label>
                </dt>
                <dd class="opt">

                    <select id="level" name="level">
                        <?php foreach ($output['level'] as $k=>$v):?>
                            <option value="<?=$k?>" <?php if ($output['page']['level'] == $k){ echo 'selected'; } ?>><?=$v?></option>
                        <?php endforeach;?>
                    </select>
            ...
            <dl class="row">
                ...
                <dd class="opt">
                    ...
                    <select id="type" name="type" onchange="ratio_set(this.value)">
            ...
            (
                {
                    ...
                    {
                        if (data.status == 1){
                            ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?con=common&fun=pic_cut&type=member&x=120&y=120&resize=1&ratio='+ratio+'&url='+data.url,690);
                        }else{
                            alert(data.msg);
                        }
