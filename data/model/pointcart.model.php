...
class pointcartModel extends Model {
     ...
     */
    public function getCartGoodsList($member_id,$plist=''){
        $return_array = array();
        //获取礼品购物车内信息
        if(!empty($plist)){
            $gc = '';
            $cart_info = explode(',', $plist);
            foreach($cart_info as $vt){
                $glist = explode('|', $vt);
                $gc.= $glist[0].',';
            }
            $gc = rtrim($gc,',');
             $cartgoodslist_tmp = $this->getPointCartList(array('pmember_id'=>$member_id,'pcart_id'=>array('in',$gc)));
        }else{
