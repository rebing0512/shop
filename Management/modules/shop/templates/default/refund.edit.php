...
<div class="page">
  ...
  <form id="post_form" method="post" action="index.php?con=refund&fun=edit&refund_id=<?php echo $output['refund']['refund_id']; ?>">
      ...
      <dl class="row">
        ...
        <dd class="opt"><?php echo ncPriceFormat($output['order']['pay_amount']);?></dd>
          ...
          <dl class="row">
            <dt class="tit"></dt>
            <dd class="opt">
                <a href="<?php echo ADMIN_SITE_URL;?>/index.php?con=refund&fun=alipay&refund_id=<?php echo $output['refund']['refund_id']; ?>" target="_blank">支付宝退款</a>  
                <a href="JavaScript:void(0);" class="ncap-btn-big ncap-btn-green" id="alipayBtn">退款查询</a>
            </dd>
          </dl>
          <?php } ?>
     <!--      <?php if ($output['detail_array']['refund_code'] == 'jdpay') { ?>
          <dl class="row">
            <dt class="tit"></dt>
            <dd class="opt">
                <a href="<?php echo ADMIN_SITE_URL;?>/index.php?con=refund&fun=jdpay&refund_id=<?php echo $output['refund']['refund_id']; ?>" target="_blank">京东退款</a>  
                <a href="JavaScript:void(0);" class="ncap-btn-big ncap-btn-green" id="alipayBtn">退款查询</a>
            </dd>
          </dl>
          <?php } ?> -->
...
$(function(){
	...
	$("#wxpayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?con=refund&fun=wxpay&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
	$("#alipayBtn").click(function(){
	    var ajaxurl = '<?php echo ADMIN_SITE_URL;?>/index.php?con=refund&fun=get_detail&refund_id=<?php echo $output['refund']['refund_id']; ?>';
		show_msg(ajaxurl);
	});
    $('#post_form').validate({
