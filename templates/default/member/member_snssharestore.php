...
.dialog-share .jcarousel-item a .extra { line-height: 20px; filter:progid:DXImageTransform.Microsoft.gradient(enabled='true',startColorstr='#BF02a129
02a129
02a129', endColorstr='#BF02a129
02a129
02a129');background:rgba(217,54,0,0.75); color: #FFF; display: none; width: 90px; height: 20px; position: absolute; z-index: 1; bottom: 0; left: 0;}
...
.dialog-share .bottom .button { color: #FFF; background-color: #0094DE; height: 28px; width: 80px; border: solid 1px #B22D00; cursor: pointer;}
...
<div class="dialog-share">
  <form method="post" action="index.php?con=member_snsindex&fun=sharestore&<?php if($_GET['irefresh'] == 1){?>irefresh=1<?php }?>" id="sharestore_form">
    ...
    <div class="share-widget">同步分享到：
        <ul class="s-app">
          ...
          <?php }?>
          <?php }?>
          <div class="tip"><span class="arrow"></span>点亮图标分享商品到热门社交网络去~<a target="_blank" href="<?php echo urlShop('member_sharemanage');?>"><?php echo $lang['sharebind_alsosharesetting'];?></a></div>
          <?php }?>
        </ul>
      </div>
...
$(function(){
	...
	$("#weibobtn_store").click(function(){
		if($("#sharestore_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#ss_seccode").css('display') == 'none'){
				...
				$("#ss_seccode").find("[name='codeimage']").attr('src','index.php?con=seccode&fun=makecode&nchash='+nchash+'&t=' + Math.random());
	...
	$("[nc_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){//已经绑定
		    ...
		    window.open(SHOP_SITE_URL + '/api.php?con=sharebind&type='+data_str.apikey);
		}
	});
	$("[nc_type='finishbtn']").live('click',function(){
	    ...
	    eval( "data_str = "+data_str);
		//验证是否绑定成功
		var url = '<?php echo urlShop('member_sharemanage', 'checkbind');?>';
