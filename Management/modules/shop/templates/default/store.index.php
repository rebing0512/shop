...
<script type="text/javascript">
 //勾差编辑
  function table_list_toggle(obj , url){
    var id  = $(obj).attr('list_id');
    var val = $(obj).attr('list_val');
    ...
    $.post(url , {'id':id,'key':key,'dst':dst} , function(res){
        if(res != 1)
            return false;
        obj.src = yes_to_no(obj.src);
        $(obj).attr('list_val' , dst);
        // alert('更改成功!');
    });
}
    function yes_to_no(str){
    var arr = str.split('/');
    var gif = arr.pop();
    var base = arr.join('/');
    if(gif == 'yes.png')
  ...
  var ajax_query = "index.php?con=store&fun=store_ajax";
  $(function(){
    $('body').on('click','.table_list_toggle',function(event){
      table_list_toggle(this , ajax_query);
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=store&fun=get_xml',
        colModel : [
            ...
            {display: '推荐', name : 'store_recommended', width : 150, sortable : false, align: 'left'},
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=store&fun=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=store&fun=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_operations(name, bDiv) {
    if (name == 'shopwwi_add') {
        window.location.href = 'index.php?con=store&fun=shopwwi_add';
    }
}
...
{
    ajax_form('url_adv', '<?php echo '复制URL';?>', 'index.php?con=store&fun=url_copy&store_id='+id);
}
</script>
