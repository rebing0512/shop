...
class buy_1Logic {
    ...
    /**
     * 直接购买时，判断商品是不是正在特卖中，如果是，按特卖价格计算，购买数量若超过特卖规定的上限，则按特卖上限计算
     * @param array $goods_info
     */
    ...
    private function _parseMansongRuleDesc($rule_info) {
        ...
        if (!empty($rule_info['mansong_goods_name']) && !empty($rule_info['goods_storage'])) {
            if (APP_ID == 'wap') {
                $goods_desc = " 送<a href='".urlMobile('goods','detail',array('goods_id'=>$rule_info['goods_id']))."' title='{$rule_info['mansong_goods_name']}' target='_blank'>[赠品]</a>";
            } else {
