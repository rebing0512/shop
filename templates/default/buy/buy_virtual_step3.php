...
<div class="ncc-main">
  <div class="ncc-title">
    <h3>在线支付</h3>
    <h5><a href="index.php?con=member_vr_order&fun=show_order&order_id=<?php echo $output['order_info']['order_id'];?>" target="_blank">查看订单详情</a></h5>
  </div>
  <form action="index.php?con=payment&fun=vr_order" method="POST" id="buy_form">
   ...
   还需在线支付 <strong id="api_pay_amount"><?php echo ncPriceFormat($output['pay']['payd_diff_amount']);?></strong>元。</dd>
      <?php } ?>
        ...
        <dd>（同时勾选时，系统将优先使用充值卡，不足时扣除预存款，目前还需在线支付 <strong id="api_pay_amount"><?php echo ncPriceFormat($output['pay']['pay_amount_online']);?></strong>元。）余额不足？<a href="<?php echo urlShop('predeposit', 'pd_log_list');?>" class="predeposit">马上充值</a></dd>
        ...
        <?php if (!$output['pay']['member_paypwd']) {?>
              还未设置支付密码，<a href="<?php echo urlShop('member_security', 'auth', array('type' => 'modify_paypwd'));?>" target="_blank">马上设置</a>
        <?php } ?>
        </dd>
      <?php } ?>
      </dl>
    ...
    <div class="ncc-receipt-info">
      ...
      <div class="nopay"><?php echo $lang['cart_step2_paymentnull_1']; ?> <a href="<?php echo SHOP_SITE_URL;?>/index.php?con=member_message&fun=sendmsg&member_id=<?php echo $output['order']['seller_id'];?>"><?php echo $lang['cart_step2_paymentnull_2'];?></a> <?php echo $lang['cart_step2_paymentnull_3'];?></div>
      <?php } else {?>
      <div class="ncc-receipt-info-title">
        <h3>支付选择</h3>
      </div>
