...
function drop_pcart_item(pcart_id){
    ...
    $.getJSON('index.php?con=pointcart&fun=drop&pc_id=' + pcart_id, function(data){
    	if(data.done){
...
function pcart_change_quantity(pcart_id, input){
    ...
    var _v = input.value;
    $.getJSON('index.php?con=pointcart&fun=update&pc_id=' + pcart_id + '&quantity=' + _v, function(result){
