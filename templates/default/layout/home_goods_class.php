<?php defined( 'TTShop') or exit( 'Access Invalid!');?>
    <div class="title">
        ...
        <h3>
            <a href="<?php echo urlShop('category', 'index');?>">
                <?php echo $lang[ 'nc_all_goods_class'];?>
            </a>
        </h3>
    </div>
        ...
        <ul class="menu">
            <?php if (!empty($output[ 'show_goods_class']) && is_array($output['show_goods_class'])) { $i=0 ; ?>
                <?php foreach ($output[ 'show_goods_class'] as $key=>$val) { $i++; ?>
                    <li cat_id="<?php echo $val['gc_id'];?>" class="<?php echo $i%2==1 ? 'odd':'even';?>"
                    <?php if($i>14){?>style="display:none;"<?php }?>>
                            <div class="class">
                                <?php if($val[ 'cn_pic'] !='' ) { ?>
                                    <span class="ico">
                                        <img src="<?php echo $val['cn_pic'];?>">
                                    </span>
                                    <?php } ?>
                                        <h4>
                                            <a href=" <?php if (!empty($val['channel_id']))echo urlShop('channel','index',array('id'=> $val['channel_id']));else echo urlShop('search','index',array('cate_id'=> $val['gc_id']));?>">
                                             ...
                                             <span class="recommend-class">
                                              ...
                                              <?php $a = 0; foreach ($val['cn_classs'] as $k => $v) {$a++; ?>
                                              <?php if ($a < 5) { ?> 
                            ...
                            <div class="sub-class" cat_menu_id="<?php echo $val['gc_id'];?>">
                                <div class="sub-class-content">
                                    ...
                                    <?php if (!empty($val[ 'class2']) && is_array($val[ 'class2'])) { ?>
                                        <?php foreach ($val[ 'class2'] as $k=>
                                            $v) { ?>
                                            <dl>
                                                <dt>
                                                    <h3>
                                                        <a href="<?php if (!empty($v['channel_id']))echo urlShop('channel','index',array('id'=> $v['channel_id']));else echo urlShop('search','index',array('cate_id'=> $v['gc_id']));?>">
                                                            <?php echo str_cut($v[ 'gc_name'],10);?>
                                                        </a>
                                                    </h3>
                                                </dt>
                                                <dd class="goods-class">
                                                    <?php if (!empty($v[ 'class3']) && is_array($v[ 'class3'])) { ?>
                                                        <?php foreach ($v[ 'class3'] as $k3=>
                                                            $v3) { ?>
                                        ...
                                        <?php }?>
                                            <div class="adv-promotions">
                                                <?php if($val[ 'cn_adv1'] !='' ) { ?>
                                                    ...
                                                    <?php } ?>
                                                        <?php if($val[ 'cn_adv2'] !='' ) { ?>
                                                            <a <?php echo $val[ 'cn_adv2_link']=='' ? 'href="javascript:;"' :
                                                            'target="_blank" href="'.$val[ 'cn_adv2_link']. '"';?>
                                                                >
                                                                <img src="<?php echo $val['cn_adv2'];?>">
                                                            </a>
                                                            <?php } ?>
                                            </div>
                                </div>
