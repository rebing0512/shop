...
<div class="page">
  ...
  <div class="explanation" id="explanation">
    ...
    <ul>
      <li>特卖分类最多为2级分类，商家发布特卖活动时选择分类，用于特卖聚合页对特卖活动进行筛选</li>
    </ul>
  </div>
      ...
      <tbody>
        ...
        <tr class="<?php echo $val['class_parent_id'] == '0' ? '' : 'two'; ?> <?php echo 'parent' . $val['class_parent_id']; ?>" data-id="<?php echo $val['class_id']; ?>">
          ...
          <td class="handle"><a href="javascript:;" onclick="submit_delete(<?php echo $val['class_id']; ?>)" class="btn red"><i class="fa fa-trash-o"></i>删除</a>
            ...
            <a href="index.php?con=groupbuy&fun=class_add&parent_id=<?php echo $val['class_id']; ?>" class="btn green"><i class="fa fa-plus"></i><?php echo $lang['nc_add_sub_class']; ?></a>
            <?php } ?></td>
...
function submit_delete(id) {
    if (confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        ...
        $('#list_form').attr('action','index.php?con=groupbuy&fun=class_drop');
        $('#class_id').val(id);
        $('#list_form').submit();
    }
}
