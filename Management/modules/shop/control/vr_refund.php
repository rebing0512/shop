...
class vr_refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'con=vr_refund&fun=refund_manage','text'=>'待处理'),
            array('url'=>'con=vr_refund&fun=refund_all','text'=>'所有记录'),
    );
    ...
    public function get_all_xmlOp() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            if ($_GET['waiting'] == 1 && $refund_info['admin_state'] == 1) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?con=vr_refund&fun=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?con=vr_refund&fun=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?con=vr_refund&fun=refund_manage');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
