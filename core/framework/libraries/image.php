...
Class Image {
	...
	Static Public function verify ($length = NULL, $width = NULL, $height = NULL) {
		...
		$len = strlen($seed) - 1;
		for ($i = 0; $i < 20; $i++) {
			$color = imagecolorallocate($verify, mt_rand(120, 255), mt_rand(120, 255), mt_rand(120, 255));
	 ...
	 * @return [Boolean]
	 */
	public static function thumb($img, $width,$height,$save_name,$save=NULL)
    {
        if (!self::checkCondition($img))
        {
            throw_exception("invalid source image");
            exit;
        }

        $width = $width ? $width : THUMB_WIDTH;
        $height = $height ? $height : THUMB_HEIGHT;
        ...
        imagecopyresampled($thumb,$image,0,0,$sx,$sy,$width,$height,$thumbw,$thumbh);

        if ($type == 'gif')
            imagecolortransparent($thumb,$color);

		//保存路径
		$save = self::savePath($save, $img,$save_name);
		$fn = 'image' . $type;
		$fn($thumb, $save);
...
//	Static Public function thumb ($img, $width = '', $height = '',$save_name, $save = NULL) {
//		if (!self::checkCondition($img)) return false;
//
//		//初始化参数(放进框架时把常量换成 读取配置文件项 )
...
//	}
	 ...
	 * @return [String]
	 */
	Static Private function savePath ($save, $img,$save_name) {
		if (!$save) return $img;
		...
		if($save_name){
			return $path . $save_name . $pathInfo['extension'];
		}else{
