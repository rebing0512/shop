...
class memberControl extends SystemControl{
    ...
    public function member_editOp(){
        ...
        if (chksubmit()){
                ...
                if ($result){
                    $url = array(
                    array(
                    'url'=>'index.php?con=member&fun=member',
                    'msg'=>$lang['member_edit_back_to_list'],
                    ),
                    array(
                    'url'=>'index.php?con=member&fun=member_edit&member_id='.intval($_POST['member_id']),
    ...
    public function member_addOp(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $insert_array = array();
                $insert_array['vip']            = trim($_POST['vip']);
                ...
                if ($result){
                    ...
                    array(
                    'url'=>'index.php?con=member&fun=member_add',
    ...
    public function get_xmlOp() {
        ...
        $order = 'vip desc';
        $param = array('member_id','member_name','member_avatar','member_email','login_mobile','member_sex','member_truename','member_birthday'
                ,'member_time','member_login_time','member_login_ip','member_points','member_exppoints','member_grade','available_predeposit'
                ,'freeze_predeposit','available_rc_balance','freeze_rc_balance','inform_allow','is_buy','is_allowtalk','member_state','vip'
        );
        ...
        foreach ($member_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn blue' href='index.php?con=member&fun=member_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            ...
            $param['vip'] = $value['vip'] == 1 ? '是' : '否';
            $param['member_email'] = $value['member_email'];
            $param['login_mobile'] = $value['login_mobile'];
