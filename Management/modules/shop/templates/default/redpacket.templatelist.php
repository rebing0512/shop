...
<script>
$(function(){
    var flexUrl = 'index.php?con=redpacket&fun=rptlist_xml';
...
function fg_operate(name, bDiv) {
	if (name == 'add') {
		window.location.href = 'index.php?con=redpacket&fun=rptadd';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除吗？')){
        $.getJSON('index.php?con=redpacket&fun=rptdel', {tid:id}, function(data){
            if (data.state) {
