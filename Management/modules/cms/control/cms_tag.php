...
Tpl::showpage('cms_tag.add');
    ...
    public function cms_tag_saveOp() {
        ...
        if($result) {
            ...
            showMessage(Language::get('tag_add_success'),'index.php?con=cms_tag&fun=cms_tag_list');
        } else {
            $this->log(Language::get('cms_log_tag_save').$result, 0);
            showMessage(Language::get('tag_add_fail'),'index.php?con=cms_tag&fun=cms_tag_list','','error');
        }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_list'),'menu_url'=>'index.php?con=cms_tag&fun=cms_tag_list'),
            'add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?con=cms_tag&fun=cms_tag_add'),
        );
