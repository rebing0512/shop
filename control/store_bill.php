...
class store_billControl extends BaseSellerControl {
    ...
    public function show_billOp(){
        ...
        if ($_GET['type'] =='refund'){
            ...
            $refund_list = $model_refund->getRefundReturnList($refund_condition,20,'s_refund_return.*,ROUND(refund_amount*commis_rate/100,2) as commis_amount');
    ...
    public function export_orderOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('murl','index.php?con=store_bill&fun=show_bill&ob_id='.$_GET['ob_id']);
    ...
    public function export_refund_orderOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?con=store_bill&fun=show_bill&query_type=refund&ob_id='.$_GET['ob_id']);
                Tpl::showpage('store_export.excel');
                exit();
            }else{
                //如果数量小，直接下载
                $data = $model_refund->getRefundReturnList($condition,'','s_refund_return.*,ROUND(s_refund_amount*commis_rate/100,2) as commis_amount',self::EXPORT_SIZE);
            }
        }else{
            //下载
            $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_refund->getRefundReturnList($condition,'','s_refund_return.*,ROUND(s_refund_amount*commis_rate/100,2) as commis_amount',"{$limit1},{$limit2}");
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'实物订单结算', 'menu_url'=>'index.php?con=bill&fun=list'),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>'index.php?con=store_bill&fun=show_bill&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'refund_list','menu_name'=>'退款订单','menu_url'=>'index.php?con=store_bill&fun=show_bill&type=refund&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'cost_list','menu_name'=>'促销费用','menu_url'=>'index.php?con=store_bill&fun=show_bill&type=cost&ob_id='.$_GET['ob_id'])
                );
                if (floatval($this->_bill_info['ob_order_book_totals']) > 0) {
                    array_push($menu_array,array('menu_key'=>'book_list','menu_name'=>'未退定金', 'menu_url'=>'index.php?con=store_bill&fun=show_bill&type=book&ob_id='.$_GET['ob_id']));
                }
                break;
        }
