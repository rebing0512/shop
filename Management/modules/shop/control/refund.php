...
class refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'con=refund','text'=>'待处理'),
            array('url'=>'con=refund&fun=refund_all','text'=>'所有记录'),
            array('url'=>'con=refund&fun=reason','text'=>'退款退货原因')
    );
    ...
    public function get_manage_xmlOp() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?con=refund&fun=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?con=refund&fun=index');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
    ...
    public function add_reasonOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?con=refund&fun=reason');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
    ...
    public function del_reasonOp() {
        ...
        if ($state) {
            ...
            showMessage(Language::get('nc_common_del_succ'),'index.php?con=refund&fun=reason');
        } else {
