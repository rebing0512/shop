...
class member_fleaControl extends BaseMemberControl{
    ...
    public function flea_listOp() {
		...
		Tpl::output('menu_sign_url','index.php?con=member_flea');
	...
	public function save_goodsOp() {
		...
		if (!empty($_POST['form_submit']) && $_POST['form_submit'] == 'ok'){
			...
			if($state) {
				...
				showMessage($lang['store_goods_index_flea_add_success'],'index.php?con=member_flea');
			} else {
				showMessage($lang['store_goods_index_flea_add_fail'],'index.php?con=member_flea','html','error');
			}
		}
	}
	               	/**
	...
	public function drop_goodsOp() {
		...
		if($state) {
			showDialog($lang['store_goods_index_flea_del_success'],'index.php?con=member_flea','succ');
		} else {
			showDialog($lang['store_goods_index_flea_del_fail'],'index.php?con=member_flea','error');
		}
	}
	/**
	 * 编辑闲置物品页面
	...
	public function edit_save_goodsOp() {
		...
		if ($_POST['form_submit'] == 'ok' &&  $goods_id!= 0){
			...
			if($state) {
				...
				showMessage($lang['store_goods_index_flea_goods_edit_success'],'index.php?con=member_flea');
			} else {
				showMessage($lang['store_goods_index_flea_goods_edit_fail'],'index.php?con=member_flea','html','error');
			}
		}
	}
	/**
...
//				echo json_encode(array('state'=>'false','message'=>$lang['iswfupload_upload_pic_fail']));
	...
	public function favoritesOp(){
		...
		if (!empty($_GET['drop']) && $_GET['drop'] == ok && !empty($_GET['fav_id'])){
			...
			showDialog($lang['flea_favorite_del_success'],'index.php?con=member_flea&fun=favorites','succ');
		}
		/**
		 * 分页
		 */
		...
		Tpl::output('show_page',$page->show());
		Tpl::output('menu_sign_url','index.php?con=member_flea&fun=favorites');
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {
		...
		switch ($menu_type) {
			case 'goods':
				$menu_array	= array(
					1=>array('menu_key'=>'flea_list',	'menu_name'=>闲置列表,			'menu_url'=>'index.php?con=member_flea')
				);
				break;
			case 'add_goods':
				$menu_array = array(
					1=>array('menu_key'=>'flea_list',	'menu_name'=>闲置列表,			'menu_url'=>'index.php?con=member_flea'),
					2=>array('menu_key'=>'goods_add',	'menu_name'=>发布闲置,	'menu_url'=>'index.php?con=member_flea&fun=add_goods')
				);
				break;
			case 'favorites':
				$menu_array = array(
					1=>array('menu_key'=>'favorites',	'menu_name'=>编辑闲置,	'menu_url'=>'index.php?con=member_flea&fun=favorites')
				);
		}
		if(!empty($array)) {
			$menu_array[] = $array;
