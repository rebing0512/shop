...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step2Op() {
        ...
        if(!$result['state']) {
            showMessage($result['msg'], 'index.php?con=cart', 'html', 'error');
        }

        //转向到商城支付页面
        redirect('index.php?con=buy&fun=pay&pay_sn='.$result['data']['pay_sn']);
    }
    ...
    public function payOp() {
        ...
        if (!preg_match('/^\d{18}$/',$pay_sn)){
            showMessage(Language::get('cart_order_pay_not_exists'),'index.php?con=member_order','html','error');
        }
        ...
        if (empty($order_list)) {
            showMessage('未找到需要支付的订单','index.php?con=member_order','html','error');
        }
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
            redirect('index.php?con=buy&fun=pay_ok&pay_sn='.$pay_sn.'&is_chain='.$flag_chain.'&pay_amount='.ncPriceFormat($pay_amount));
        }
        ...
        if ($pay['pay_amount_online'] > 0) {
            ...
            if (empty($payment_list)) {
                showMessage('暂未找到合适的支付方式','index.php?con=member_order','html','error');
            }
    ...
    public function pd_payOp() {
        $pay_sn = $_GET['pay_sn'];
        if (!preg_match('/^\d{18}$/',$pay_sn)){
            showMessage(Language::get('para_error'),urlShop('predeposit'),'html','error');
        }
        ...
        if (intval($pd_info['pdr_payment_state'])) {
            showMessage('您的订单已经支付，请勿重复支付',urlShop('predeposit'),'html','error');
        }
        ...
        if (empty($payment_list)) {
            showMessage('暂未找到合适的支付方式',urlShop('predeposit'),'html','error');
        }
        Tpl::output('payment_list',$payment_list);
    ...
    private function _buy_branch($post) {
        if (!$post['ifcart']) {
            ...
            if ($goods_info['is_virtual']) {
                redirect('index.php?con=buy_virtual&fun=buy_step1&goods_id='.$goods_id.'&quantity='.$quantity);
            }
        }
    }
