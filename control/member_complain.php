...
class member_complainControl extends BaseMemberControl{
    ...
    public function complain_newOp() {
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showMessage(Language::get('wrong_argument'),'index.php?con=member_order&fun=index','html','error');
        }
        $condition = array();
    ...
    public function complain_saveOp() {
        ...
        if($if_complain < 1) {
            showDialog(Language::get('para_error'),'index.php?con=member_order&fun=index','error');
        }
        //检查是不是正在进行投诉
        ...
        if ($state) {
            showDialog(Language::get('complain_submit_success'),'index.php?con=member_complain&fun=index','succ');
        } else {
    ...
    public function apply_handleOp() {
        ...
        showMessage(Language::get('handle_submit_success'),'index.php?con=member_complain');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'complain_accuser_list','menu_name'=>Language::get('complain_manage_title'),'menu_url'=>'index.php?con=member_complain')
        );
