...
class store_goods_addControl extends BaseSellerControl
{
    ...
    private function checkStore()
    {
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?con=store_goods_online&fun=goods_list', 'html', 'error');
            }
        }
    }
    ...
    public function add_step_oneOp()
    {
        ...
        // 商品分类
//        $goods_class = $model_goodsclass->getGoodsClass($_SESSION['store_id'], 0, 1, $_SESSION['seller_group_id'], $_SESSION['seller_gc_limits'], $this->store_info['bind_all_gc']);
        header("Location: ?con=store_goods_add&fun=add_step_two");
        exit;
...
//        Tpl::showpage('store_goods_add.step1');
    ...
    public function add_step_twoOp()
    {
        ...
        $data = $systemcategoryModel::getSystemCategory();
        Tpl::output('data', $data['result']['category']);
        // 实例化商品分类模型
        $model_goodsclass = Model('goods_class');
...
//        }
    ...
    public function ajax_goods_classOp()
    {
        ...
        $model_goodsclass = Model('goods_class');
        $list = $model_goodsclass->getGoodsClass($_SESSION['store_id'], $gc_id, $deep, $_SESSION['seller_group_id'], $_SESSION['seller_gc_limits'], $this->store_info['bind_all_gc']);
