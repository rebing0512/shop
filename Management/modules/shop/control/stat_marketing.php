...
class stat_marketingControl extends SystemControl{
    private $links = array(
        array('url'=>'con=stat_marketing&fun=promotion','lang'=>'stat_promotion'),
        array('url'=>'con=stat_marketing&fun=group','lang'=>'stat_group'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['fun']) || in_array($this->search_arr['fun'],array('promotion','group','get_groupgoods_xml','groupgoods','get_grouplist_xml'))){
    ...
    public function promotionOp(){
        ...
        //优惠类型数组
        $goodstype_arr = array(2=>'特卖',3=>'限时折扣',4=>'优惠套装');
        $statlist = array();
    ...
    public function groupgoodsOp(){
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
            ...
            $excel_obj->addWorksheet($excel_obj->charset('特卖商品统计',CHARSET));
            $excel_obj->generateXML($excel_obj->charset('特卖商品统计',CHARSET).date('Y-m-d-H',time()));
            exit();
        }
    }
    /**
