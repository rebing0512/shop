...
class orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        ...
        //追加返回分销消息 20160906
        if (in_array('distribution', $extend)) {
            $model_mingxi = Model('mingxi');
            foreach ($order_list as $order_id => $value) {
                $fengcheng_total = 0;
                $condition = array();
                $condition['order_sn'] = $value['order_sn'];
                $mingxi_list = $model_mingxi->getMingxiList($condition);
                if ($mingxi_list) {
                    foreach ($mingxi_list as $mingxi) {
                        $fengcheng_total += $mingxi['je'];
                    }           
                }
                $order_list[$order_id]['fengcheng_total'] = ncPriceFormat($fengcheng_total);
            }
        }
        return $order_info;
    }
     ...
     * 买家代发货
     */
    public function getOrderStateWaitShipping($condition=array())
    {
    ...
    public function getOrderStateCancelCount($condition = array()) {
        $condition['order_state'] = array('in',array(ORDER_STATE_CANCEL));
        return $this->getOrderCount($condition);
    }
     ...
     * 取得订单商品实际佣金 20160906
     */
    public function getOrderGoodsCommisRate($goods_info) {
        $model_store_bind_class = Model('store_bind_class');

        $goods_list = array($goods_info);
        $result = $model_store_bind_class->getStoreGcidCommisRateList($goods_list);
        $commis_rate = $result[$goods_info['store_id']][$goods_info['gc_id']];
        ...
        $condition['rec_id'] = $goods_info['rec_id'];
        $this->editOrderGoods($data, $condition);
