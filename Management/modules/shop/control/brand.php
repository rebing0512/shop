...
class brandControl extends SystemControl{
    ...
    public function get_xmlOp() {
        ...
        foreach ($brand_list as $value) {
            $param = array();
			$operation = "<a class='btn red' href='javascript:void(0);' onclick=\"fg_delete(".$value['brand_id'].")\"><i class='fa fa-trash-o'></i>删除</a>";
            if ($_GET['type'] == 'apply') {
                $operation .= "<a class='btn orange' href='javascript:void(0)' onclick=\"fg_apply(".$value['brand_id'].")\"><i class='fa fa-check-square'></i>审核</a>";
            } else {
                $operation .= "<a class='btn blue' href='index.php?con=brand&fun=brand_edit&brand_id=" . $value['brand_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                $insert_array['show_type'] = intval($_POST['show_type'])==1?1:0;
                 //上传网站band大圖
                if (!empty($_FILES['brand_bigpic']['name'])){
                    $upload = new UploadFile();
                    $upload->set('default_dir',ATTACH_BRAND_BIG);
                    $result = $upload->upfile('brand_bigpic');
                    if ($result){
                        $_POST['brand_bigpic'] = $upload->file_name;
                    }else {
                        showMessage($upload->error,'','','error');
                    }
                }
                $insert_array['brand_bigpic'] = $_POST['brand_bigpic'];
                $result = $model_brand->addBrand($insert_array);
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?con=brand&fun=brand_add',
                            'msg'=>$lang['brand_add_again'],
                        ),
                        array(
                            'url'=>'index.php?con=brand&fun=brand',
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if (!empty($_FILES['brand_bigpic']['name'])){
                    ...
                    $update_array['brand_bigpic'] = $_POST['brand_bigpic'];
                }else{

                     $update_array['brand_bigpic'] = trim($_POST['pic_ori']);
                ...
                if ($result){
                    ...
                    //判断有没有之前的图片，如果有则删除
                    if (!empty($brand_info['brand_bigpic']) && !empty($_POST['brand_bigpic'])){
                        @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ARTICLE_LOGO.DS.$brand_info['brand_bigpic']);
                    }
                    $url = array(
                        array(
                            'url'=>'index.php?con=brand&fun=brand_edit&brand_id='.intval($_POST['brand_id']),
    ...
    public function brand_delOp(){
        ...
        if (preg_match('/^[\d,]+$/', $_GET['del_id'])) {
            ...
            foreach ($_GET['del_id'] as $k => $v){
                $v = intval($v);
                Model('brand')->delBrand(array('brand_id' => $v));
            }
            $this->log(L('nc_delete,brand_index_brand').'[ID:'.implode(',',$_GET['del_id']).']',1);
            exit(json_encode(array('state'=>true,'msg'=>'删除成功')));
        } else {
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }
    ...
    public function export_step1Op(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?con=brand&fun=brand');
