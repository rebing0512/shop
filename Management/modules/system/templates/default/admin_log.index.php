...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=admin_log&fun=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=admin_log&fun=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?con=admin_log&fun=get_xml',
        colModel : [
    ...
    if (name == 'csv') {
    	...
    	if(confirm('删除后将不能恢复，确认删除吗？')){
    		$.ajax({
    	        type: "GET",
    	        dataType: "json",
    	        url: "index.php?con=admin_log&fun=list_del",
    	        data: "type=ago",
