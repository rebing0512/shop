...
<?php //if ($output['isOwnShop'] || (!empty($output['fcode_quota']) && $output['fcode_quota']['fcq_endtime'] > TIMESTAMP)) { ?>
    ...
    <table class="ncsc-default-table">
        ...
        <tr nctype="tr_no_promotion" style="display:none;">
            <td colspan="20" class="norecord"><div class="no-promotion"><i class="zw"></i><span>好物推荐商品列表暂无内容，请选择添加平台首页展位商品。</span></div></td>
        </tr>
...
<script>
    $(function(){
        ...
        $('input[nctype="btn_search_goods"]').click(function(){
            _url = '<?php echo urlShop('hot_goods_recommend', 'hot_select_goods');?>';
        ...
        $('div[nctype="div_goods_select"]').on('click', 'a[nctype="a_choose_goods"]', function(){
            ...
            CUR_DIALOG =ajax_form('choose_goods', '好物推荐设定', '<?php echo urlShop('hot_goods_recommend', 'choosed_goods');?>&gid='+_gid, 640);
        });

        // 删除商品
        $('tbody[nctype="choose_goods_list"]').on('click','a[nctype="del_choosed"]', function(){
            $this = $(this);
            _url = '<?php echo urlShop('hot_goods_recommend', 'del_choosed_goods');?>';
