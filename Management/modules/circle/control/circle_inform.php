...
Tpl::showpage('circle_inform');
    ...
    private function spellInformUrl($param){
        if($param['reply_id'] == 0) return $url = 'index.php?con=theme&fun=theme_detail&c_id='.$param['circle_id'].'&t_id='.$param['theme_id'];
        ...
        $page = ceil($count/15);
        return $url = 'index.php?con=theme&fun=theme_detail&c_id='.$param['circle_id'].'&t_id='.$param['theme_id'].'&curpage='.$page.'#f'.$param['reply_id'];
    }
    /**
     * 举报状态
     */
