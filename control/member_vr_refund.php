...
class member_vr_refundControl extends BaseMemberControl {
    ...
    public function add_refundOp(){
        ...
        if ($order_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?con=member_vr_refund&fun=index','error');
        }
        $condition = array();
        ...
        if (!$order['if_refund']) {//检查状态,防止页面刷新不及时造成数据错误
            showDialog(Language::get('wrong_argument'),'index.php?con=member_vr_order&fun=index','error');
        }
        if (chksubmit() && $order['if_refund']){
            ...
            if ($state) {
                showDialog(Language::get('nc_common_save_succ'),'index.php?con=member_vr_refund&fun=index','succ');
            } else {
