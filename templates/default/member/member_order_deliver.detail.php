...
<div class="ncm-flow-layout">
  ...
  <div class="ncm-flow-item">
    ...
    <div class="item-goods">
      ...
      <dl>
        <dt>
          <div class="ncm-goods-thumb-mini"><a href="index.php?con=goods&goods_id=<?php echo $val['goods_id']; ?>" target="_blank"><img src="<?php echo thumb($val,'60'); ?>" onMouseOver="toolTip('<img src=<?php echo thumb($val,'240'); ?>>')" onMouseOut="toolTip()"/></a></div>
        </dt>
        <dd><a href="index.php?con=goods&goods_id=<?php echo $val['goods_id']; ?>" target="_blank"><?php echo $val['goods_name']; ?></a> <span class="rmb-price"><?php echo ncPriceFormat($val['goods_price']); ?>&nbsp;*&nbsp;<?php echo $val['goods_num']; ?>
...
$(function(){
  //Ajax提示
  $.getJSON('index.php?con=member_order&fun=get_express&e_code=<?php echo $output['e_code']?>&shipping_code=<?php echo $output['order_info']['shipping_code']?>',function(data){
    if(data){
      $('#express_list').html('<iframe src="'+data+'" width="580"height="380">').next().css('display','');
    }else{
