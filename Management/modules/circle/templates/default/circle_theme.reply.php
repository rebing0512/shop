  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        <li><a href="index.php?con=circle_theme&fun=theme_list"><?php echo $lang['circle_theme_list'];?></a></li>
        <li><a href="index.php?con=circle_theme&fun=theme_info&t_id=<?php echo $output['t_id'];?>"><?php echo $lang['circle_theme_info'];?></a></li>
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=circle_theme&fun=get_reply_xml&t_id=<?php echo $_GET['t_id'];?>',
        colModel : [
...
function fg_del(t_id, r_id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?con=circle_theme&fun=theme_replydel', {t_id:t_id, r_id:r_id}, function(data){
            if (data.state) {
...
function fg_recommend(id, value) {
    ...
    $.getJSON('index.php?con=circle_theme&fun=theme_recommend', {id:id, value:value}, function(data){
        if (data.state) {
            $("#flexigrid").flexReload();
        } else {
            showError(data.msg)
