...
class SystemControl{
    ...
    protected final function systemLogin() {
        ...
        if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
            @header('Location: '. ADMIN_SITE_URL .'/index.php?con=login&fun=login');exit;
        }else {
    ...
    protected final function only(array $array)
    {
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        $act = $_GET['con'] ? $_GET['con'] : $_POST['con'];
        $act = $act == '' ? 'index' : $act;
        $permission = $this->getPermission();
//var_dump($permission[MODULE_NAME]);exit;
        if ($permission[MODULE_NAME]){
            foreach ($permission[MODULE_NAME] as $k => $v){
                if (stripos($v,'&amp;')){
                    $permission[MODULE_NAME][$k] = strstr($v,'&amp;',TRUE);
                }
            }
        }
        //return true;
        ...
        if (!empty($permission)){
            foreach ($permission as $key => $value){
                foreach ($value as $k => $v){
                    $permission[$key][$k] = str_replace('&amp;','&',$v);
                }
            }
        }
    ...
    protected final function sublink($links = array(), $actived = '', $file='index.php') {
        ...
        foreach ($links as $k=>$v) {
            ...
            if (empty($array['fun'])) $array['fun'] = 'index';

            if (!$this->checkPermission($array)) continue;
            $href = ($array['fun'] == $actived ? null : "href=\"{$file}?{$v['url']}\"");
            $class = ($array['fun'] == $actived ? "class=\"current\"" : null);
            if ((count($array)>2)&&($class==null)){
                $class = (array_values($array)[2] == $actived ? "class=\"current\"" : null);
            }
    ...
    protected final function log($lang = '', $state = 1, $admin_name = '', $admin_id = 0) {
        ...
        $data['ip']         = getIp();
        $data['url']        = $_REQUEST['con'].'&'.$_REQUEST['fun'];
