...
class Sms {
    ...
    public function send($mobile,$content) {
       $control_sms_type=C('control_sms_type');
        if($control_sms_type==1)
        {
            return $this->mysend_smsbao($mobile,$content);
        }
        if($control_sms_type==2)
        {
...
/*
    ...
    */
    private function mysend_smsbao($mobile,$content){
       ...
       $user_id = urlencode(C('control_sms_zh')); // 这里填写用户名
       $pass = urlencode(C('control_sms_pw')); // 这里填登陆密码
