...
<script type="text/javascript">
          var arr = <?php echo $output['stag']; ?>;
          var ajax_query ="index.php?con=goods&fun=ajax_query";
          function table_list_select(obj , url , arr){

          if($(obj).find('select').length != 0)
            return false;
            
            var id    = $(obj).attr('list_id');
            var key   = $(obj).attr('list_key');
            var val   = $(obj).html();
            $(obj).html('');
            $(obj).append('<select style="width:100px;"></select>');
            ...
            select.change(function(){
              ...
              $.post(url , {'id':id,'key':key,'dst':dst} , function(res){
        ...
        $(function(){
           $('body').on('click','.table_list_select',function($event){
              table_list_select(this , ajax_query , arr);
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=goods&fun=get_xml&type=<?php echo $output['type'];?>',
        colModel : [
            {display: '操作', name : 'operation', width : 150, sortable : false, align: 'center', className: 'handle'},
            {display: '推荐', name : 'tag', width : 60, sortable : false, align: 'center'},
            ...
            {display: '访问量', name : 'goods_click', width : 100, sortable : true, align: 'center'},
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=goods&fun=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?con=goods&fun=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_lonkup(ids) {
    _uri = "index.php?con=goods&fun=goods_lockup&id=" + ids;
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?con=goods&fun=goods_del', {id:id}, function(data){
            if (data.state) {
...
function fg_verify(ids) {
    _uri = "index.php?con=goods&fun=goods_verify&id=" + ids;
