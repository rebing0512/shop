...
<script>
    var selected_category = '<?php echo $output['goods']['category_id']; ?>';
    var selected_pinlei = '<?php echo $output['goods']['kind_id']; ?>';
    var selected_caizhi = '<?php echo $output['goods']['texture_id']; ?>';

    $(function () {
        var data = <?php echo json_encode($output['data']);?>;
        ...
        $fenlei.on('change',function(){
            ...
            $.each(data,function(i,v){
                if (v.id == val) {
                    // each caizhi
                    $.each(v.textures,function(i,v){
                        if (v.sub_name != ''){
                            var name = v.sub_name+'（'+v.name+'）';
                        }else {
                            var name = v.name
                        }
                        var html = '<option value="'+v.id+'">'+name+'</option>';
                        $(html).appendTo($caizhi);
                    });
                    // each pinlei
                    $.each(v.kinds,function(i,k){
                        $.each(k.attributes,function(i,v){
                            var html = '<option value="'+v.id+'">'+k.name+' - '+v.name+(v.mark?'（'+v.mark+'）':'')+'</option>';
                            $(html).appendTo($pinlei);
                        });
                    });
                }
            });
            ...
            {
                $pinlei.val(selected_pinlei);
                $caizhi.val(selected_caizhi);
            }
            var pl = $pinlei.find('option:selected').text().split(' - ')[1]
            if (pl=='其它'){
                $pinlei.after(
                    '<input name="attribute" type="text" class="text w100" value="<?php echo $output['goods']['attribute']; ?>" maxlength="5" placeholder="自定义品类名"/>'
            )
            } else {
                $('[name=attribute]').remove()
            }
            if ($caizhi.find('option:selected').text()=='其它'){
                $caizhi.after(
                    '<input name="texture" type="text" class="text w100" value="<?php echo $output['goods']['texture']; ?>"  maxlength="5" placeholder="自定义材质名"/>'
                )
            } else {
                $('[name=texture]').remove()
            }
        ...
        $.each(data,function(i,v){
            ...
            $(html).appendTo($fenlei);
        });

        if (selected_category)
        {
            $fenlei.val(selected_category);
            $fenlei.trigger('change');
        }
    });
</script>
<div id="fixedNavBar">
...
<div class="item-publish">
    <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) {
        ...
        <div class="ncsc-form-goods">
            ...
            <dl>
                <dt><i class="required">*</i>分类<?php echo $lang['nc_colon']; ?></dt>
                <dd>
                    <select name="category_id" id="fenlei">
                        <option value="">
                            --请选择--
                        </option>
                    </select>
            ...
            <dl>
                <dt><i class="required">*</i>品类<?php echo $lang['nc_colon']; ?></dt>
                <dd>
                    <select name="attribute_id" id="pinlei">
                        <option value="">
                            --请先选择分类--
                        </option>
                    </select>
            ...
            <dl>
                <dt><i class="required">*</i>材质<?php echo $lang['nc_colon']; ?></dt>
                <dd>
                    <select name="texture_id" id="caizhi">
                        <option value="">
                            --请先选择分类--
                        </option>
                    </select>
            ...
            <dl>
                <dt nc_type="no_spec"><i class="required">*</i>尺寸</dt>
                <dd nc_type="no_spec">
                    <input type="text" name="g_size" id="g_size" value="<?php echo $output['goods']['_size']; ?>" placeholder="如 10cmx10cm"/>
                </dd>
            </dl>
            <dl>
            <dl>
                ...
                <dl>
                    ...
                    <dd>
                        <input name="g_costprice"
                               value="<?php echo ncPriceFormat($output['goods']['goods_costprice']); ?>" type="text"
                               class="text w60"/><em class="add-on"><i class="icon-renminbi"></i></em> <span></span>
              ...
              <input type="checkbox" value="<?php echo $v['sp_value_name']; ?>"
                     ...
                     name="sp_val[<?php echo $k; ?>][<?php echo $v['sp_value_id'] ?>]">
                                    ...
                                    <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id']; ?>,sp_id:<?php echo $k; ?>,url:'<?php echo urlShop('store_goods_add', 'ajax_add_spec'); ?>'}">
                                        <div nctype="specAdd1"><a href="javascript:void(0);" class="ncbtn"
                                                                  nctype="specAdd"><i class="icon-plus"></i>添加规格值</a>
                                        </div>
                    ...
                    <dd class="spec-dd">
                        <div nctype="spec_div" class="spec-div">
                            <table border="0" cellpadding="0" cellspacing="0" class="spec_table">
                                ...
                                <th class="w90"><span
                                            class="red">*</span><?php echo $lang['store_goods_index_price']; ?>
                                    <div class="batch"><i class="icon-edit" title="批量操作"></i>
                                        <div class="batch-input" style="display:none;">
                                            <h6>批量设置价格：</h6>
                                            <a href="javascript:void(0)" class="close">X</a>
                                            <input name="" type="text" class="text price"/>
                                            <a href="javascript:void(0)" class="ncbtn-mini"
                                               data-type="price">设置</a><span class="arrow"></span></div>
                                    </div>
                                </th>
                                <th class="w60"><span
                                            class="red">*</span><?php echo $lang['store_goods_index_stock']; ?>
                                    <div class="batch"><i class="icon-edit" title="批量操作"></i>
                                        <div class="batch-input" style="display:none;">
                                            <h6>批量设置库存：</h6>
                                            <a href="javascript:void(0)" class="close">X</a>
                                            ...
                                            <a href="javascript:void(0)" class="ncbtn-mini"
                                               data-type="stock">设置</a><span class="arrow"></span></div>
                                    </div>
                                </th>
                                <th class="w70">预警值
                                    <div class="batch"><i class="icon-edit" title="批量操作"></i>
                                        <div class="batch-input" style="display:none;">
                                            ...
                                            <input name="" type="text" class="text stock"/>
                                            <a href="javascript:void(0)" class="ncbtn-mini"
                                               data-type="alarm">设置</a><span class="arrow"></span></div>
                                    </div>
                                </th>
                ...
                <dl>
                    ...
                    <dd nc_type="no_spec">
                        <input name="g_storage" value="<?php echo $output['goods']['g_storage']?:1; ?>" type="text"
                               class="text w60"/>
                        <span></span>
                  ...
                  <input type="file" hidefocus="true" size="1" class="input-file" name="add_album" id="add_album"
                         multiple>
                                    ...
                                    <a class="ncbtn mt5" nctype="show_desc"
                                       href="index.php?con=store_album&fun=pic_list&item=des"><i
                                ...
                                <div nctype="div_freight"
                                    ...
                                    <span id="postageName" class="transport-name"
                                          <?php if ($output['goods']['transport_title'] != '' && intval($output['goods']['transport_id'])) { ?>style="display: inline-block;"<?php } ?>><?php echo $output['goods']['transport_title']; ?></span><a
                                            ...
                                            onclick="window.open('index.php?con=store_transport&type=select')"
...
//                    checkPrice: true
            ...
            messages: {
                g_name: {
                    required: '<i class="icon-exclamation-sign"></i>商品标题名称长度至少3个字符，最长50个汉字',
                ...
                stone: {
                    min: '<i class="icon-exclamation-sign"></i>石头种类必选'
                },
                carve: {
                    min: '<i class="icon-exclamation-sign"></i>雕刻技法必选'
                },
                g_jingle: {
                    maxlength: '<i class="icon-exclamation-sign"></i>商品卖点不能超过140个字符'
                },
                category_id:{
                    required: '<i class="icon-exclamation-sign"></i>分类必选'
                },
                attribute_id:{
                    required: '<i class="icon-exclamation-sign"></i>品类必选'
                },
                texture_id:{
                    required: '<i class="icon-exclamation-sign"></i>材质必选'
                },
                g_price: {
