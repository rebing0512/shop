...
<form method="get" action="index.php">
  <table class="search-form">
    <input type="hidden" name="con" value="store_goods_online" />
...
<table class="ncsc-default-table">
  <thead>
    <tr nc_type="table_header">
      ...
      <th class="w100"><?php echo '佣金设置';?></th> <!--20160906-->
    ...
    <tr>
      ...
      <td colspan="20"><label for="all" ><?php echo $lang['nc_select_all'];?></label>
        ...
        <a href="javascript:void(0);" class="ncbtn-mini" nc_type="batchbutton" uri="<?php echo urlShop('store_goods_online', 'edit_qrcode');?>" name="commonid" confirm="你确定要更新嘛？">批量更新二维码</a> 
    </tr>
    <?php } ?>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <td><span <?php if ($output['storage_array'][$val['goods_commonid']]['alarm']) { echo 'style="color:red;"';}?>><?php echo $output['storage_array'][$val['goods_commonid']]['sum'].$lang['piece']; ?></span></td>
       ...
       <td class="align-center">
        <p>一级：<?php echo !empty($val['fencheng1']) ? $val['fencheng1'] : "0.00"; ?></p>
        <p>二级：<?php echo !empty($val['fencheng2']) ? $val['fencheng2'] : "0.00"; ?></p>
        <p>三级：<?php echo !empty($val['fencheng3']) ? $val['fencheng3'] : "0.00"; ?></p>
      </td>  
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
         <!--20160906-->
          <span>
          <a onclick="ajax_form('ajax_commision', '<?php echo '佣金设置'; ?>', '<?php echo urlShop('distribution', 'commision_setting', array('goods_commonid' => $val['goods_commonid'], 'inajax' => 1)); ?>', '480')" class="btn-bluejeans" href="javascript:;">
        ...
        <?php } else {?>
        <span class="tip" title="该商品参加特卖活动期间不能进行编辑及删除等操作,但可以编辑赠品"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlShop('store_goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-darkgray"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
