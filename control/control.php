...
class Control{
    ...
    protected function showLayout() {
		...
		//底部帮助
        $ft_catelist = Model()->table('ncms_category')->where(array('parentid'=>50))->select();
       if(is_array($ft_catelist) && !empty($ft_catelist)){ 
            foreach($ft_catelist as &$vt){
                $vt['list'] =  Model()->table("ncms_newsmodel")->where("catid = {$vt['catid']}")->field("id,catid,title,islink,url")->limit(5)->select();
            }
        }

        Tpl::output('ft_catelist',$ft_catelist);
        //查询保障服务项目
...
class BaseCmsControl extends Control {
    ...
    public function __construct(){
        ...
        Language::read('common,home_layout,cms');

        Tpl::setDir('cms');

        Tpl::setLayout('cms_layout');
        $list_setting = Model('setting')->getListSetting();
        $cms_keyword = explode(',', $list_setting['cms_keyword']);
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'property' => array('name' => '财产中心', 'child' => array(
                ...
                'member_redpacket'  => array('name' => '我的红包', 'url'=>urlShop('member_redpacket', 'index')),  
                'member_invite'     => array('name' => '我的推广', 'url' => urlShop('member_invite', 'index')),   //20160906         
			)),
        );
        return $menu_list;
    }
...
class BaseMemberOneControl extends Control {
     protected $member_info = array();   // 会员信息
    public function __construct(){

        if(!C('site_status')) halt(C('closed_reason'));

        Language::read('common,member_layout_one');
        ...
        $this->showLayout();
        Tpl::setDir('member');
        Tpl::setLayout('member_layout_one');
...
class BaseSNSControl extends Control {
    ...
    public function get_member_info() {
        ...
        if(empty($member_info)){
            showMessage(L('wrong_argument'), 'index.php?con=member_snshome', '', 'error');
        }
        //粉丝数
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                array('name' => '商品发布', 'con'=>'store_goods_add', 'fun'=>'add_step_two'),
				//array('name' => 'CSV导入', 'con'=>'taobao_import', 'fun'=>'index'),
            ...
            'order' => array('name' => '订单物流', 'child' => array(
                ...
                //array('name' => '佣金记录', 'con'=>'distribution', 'fun'=>'commision_list'),//20160906
            )),
            'promotion' => array('name' => '促销', 'child' => array(
//                array('name' => '特卖管理', 'con'=>'store_groupbuy', 'fun'=>'index'),
...
//                array('name' => '优惠套装', 'con'=>'store_promotion_bundling', 'fun'=>'bundling_list'),
                ...
                array('name' => '好物推荐', 'con' => 'hot_goods_recommend', 'fun' => 'hot_goods_list'),
...
//                array('name' => '推荐组合', 'con' => 'store_promotion_combo', 'fun' => 'index'),
                array('name' => '店铺活动', 'con' => 'store_promotion_sole', 'fun' => 'index'),
...
//                array('name' => '活动管理', 'con'=>'store_activity', 'fun'=>'store_activity'),
            ...
            'store' => array('name' => '店铺', 'child' => array(
                ...
                //array('name' => '店铺资讯', 'con'=>'store_news', 'fun'=>'index'),
