...
class sns_albumControl extends mobileMemberControl {
     ...
     * @return
     */
    public function file_upload_rzOp() {
        /**
         * 读取语言包
         */
        Language::read('sns_home');
         ...
         */
        $upload = new UploadFile();
        $upload_dir = ATTACH_STORE_JOININ;
        ...
        $img_path = $upload->getSysSetPath().$upload->file_name;
        list($width, $height, $type, $attr) = getimagesize(BASE_UPLOAD_PATH.DS.ATTACH_STORE_JOININ.DS.$img_path);
   
        $data = array();
        $data['file_id'] = $result;
        $data['file_name'] = $img_path;
        $data['origin_file_name'] = $_FILES["file"]["name"];
        $data['file_url'] = rzThumb($img_path, 240);
        output_data($data);
    }
