    ...
    public function deleteOp() {
        ...
        if (preg_match('/^[\d,]+$/', $_GET['del_id'])) {
            ...
            foreach ($_GET['del_id'] as $k => $v){
                ...
                $thumb = $model_article->getOneArticle($v);

                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ARTICLE.DS.$thumb['article_thumb']);
                @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ARTICLE.DS.$thumb['article_img']);
    ...
    public function get_xmlOp(){
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?con=article&fun=article_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                $insert_array['article_goods_id'] = trim($_POST['article_goods_id']);
                $insert_array['article_view'] = trim($_POST['article_view']);
                $insert_array['article_zan'] = trim($_POST['article_zan']);
                $insert_array['article_time'] =trim($_POST['article_time'])?strtotime(trim($_POST['article_time'])):time();
                ...
                $insert_array['article_img'] = $img_path;
                $insert_array['article_thumb'] = $img_path_thumb;
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?con=article&fun=article',
                            'msg'=>"{$lang['article_add_tolist']}",
                        ),
                        array(
                            'url'=>'index.php?con=article&fun=article_add&ac_id='.intval($_POST['ac_id']),
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                $update_array['article_goods_id'] = trim($_POST['article_goods_id']);
                $update_array['article_view'] = trim($_POST['article_view']);
                $update_array['article_zan'] = trim($_POST['article_zan']);
                $update_array['article_author'] = trim($_POST['article_author']);
                $update_array['article_time'] =trim($_POST['article_time'])?strtotime(trim($_POST['article_time'])):time();

              
                if(!empty($_FILES['adv_pic']['name'])){
                    $thumb = $model_article->getOneArticle(intval($_POST['article_id']));
                    @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ARTICLE.DS.$thumb['article_thumb']);
                    @unlink(BASE_UPLOAD_PATH.DS.ATTACH_ARTICLE.DS.$thumb['article_img']);
                     /*================================*/
                    $upload     = new UploadFile();
                    $upload->set('default_dir',ATTACH_ARTICLE);
                    //生成两张缩略图，宽高分别为30,300 
                    $upload->set('thumb_width',  '250'); 
                    $upload->set('thumb_height', '180');
                    //缩略图名称后面分别追加 "_tiny","_mid" 
                    $upload->set('thumb_ext', '_thumb'); 
                    //开始上传
                    $result = $upload->upfile('adv_pic');
                    ...
                    $update_array['article_img'] = $img_path;
                    $update_array['article_thumb'] = $img_path_thumb;
                 }else{
                     $update_array['article_img'] = trim($_POST['pic_ori']);
                     $update_array['article_thumb'] =trim($_POST['pic_ori2']);
                 }
                /*================================*/               
                ...
                if ($result){
                    ...
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?con=article&fun=article_edit&article_id='.intval($_POST['article_id']),
