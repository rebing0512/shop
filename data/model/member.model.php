...
class memberModel extends Model {
	...
	public function addMember($param) {
		...
		try {
			...
			$member_info['hashkey'] = $param['hashkey'];
			...
			if ($inviter_id) {
				$check_inviter = $this -> getMemberInfo(array('member_id' => $inviter_id));
				if (is_array($check_inviter) and count($check_inviter) > 0) {
					$member_info['inviter_id'] = intval($inviter_id);
				}
			}
	 ...
	 * 获取邀请人的信息 20160906
	 */
	public function getInviterInfoByMemberId($member_id) {
		$result = $this -> getMemberInfoByID($member_id);
		$inviter_id = $result['inviter_id'];
		$inviter_info = $this -> getMemberInfoByID($inviter_id);
	 ...
	 * 获取被邀请人的信息 20160906
	 */
	public function getInInfoByMemberId($member_id) {
		$result = $this -> getMemberInfoByID($member_id);

		return $inviterr_info;
	}
