...
class goodsControl extends SystemControl{
    private $links = array(
        array('url'=>'con=goods&fun=goods','text'=>'所有商品'),
        array('url'=>'con=goods&fun=lockup_list','text'=>'违规下架'),
        array('url'=>'con=goods&fun=waitverify_list','text'=>'等待审核'),
        array('url'=>'con=goods&fun=goods_set','text'=>'商品设置'),
    );
 ...
 /*                3 => 
        ...
        $stag .='{0:\'请选择\',';
        foreach ($tag as $key => $values) {
            
            $stag .=$values['val'].':\''.$values['zhname'].'\',';
        }
        $stag = rtrim($stag,',');
        $stag.='}';

        Tpl::output('tag', $tag);
        Tpl::output('stag', $stag);
    ...
    public function get_xmlOp() {
        ...
        $order = '';
        $param = array('goods_commonid', 'goods_name', 'goods_price', 'goods_click', 'goods_state', 'goods_verify', 'goods_image', 'goods_jingle', 'gc_id'
        ...
        //$goods_click = $model_goods->select();
        ...
        foreach ($goods_list as $value) {
            ...
            $operation .= "</ul>";
            $operation1 .= '<span class="table_list_select" list_id="'.$storage_array[$value['goods_commonid']]['goods_id'].'" list_key="tag">';
            ...
            foreach($tag as &$vals) {
                 ...
                 if($vals['val'] == $value['tag']){
                      $operation1.=  $vals['zhname'];
...
//            foreach ($goods_click as $item){
//                if ($item['goods_commonid'] == $value['goods_commonid']){
//                    $param['goods_click'] = $item['goods_click'];
//                }
//            }
            $param['goods_click'] = $value['goods_click'];
    ...
    *设置tag
    */
    public function ajax_queryOp(){
        $common_id = intval($_POST['id']);
       
        $fin_id = Model('goods')->where(array('goods_id'=>$common_id))->find();
        $model = Model();
        $where['goods_commonid'] = $fin_id['goods_commonid'];
        $save[$_POST['key']] = $_POST['dst'];
        $up = $model->table('goods')->where(array('goods_id'=>$common_id))->update($save);
        $up = $model->table('goods_common')->where($where)->update($save);
        echo 1;
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?con=goods&fun=index');
