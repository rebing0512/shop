...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?con=goods_recommend&fun=index" title="返回商品分类推荐列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
  ...
  <form id="goods_form" method="post" action='index.php?con=goods_recommend&fun=save'>
    ...
    <div class="ncap-form-default" id="explanation">
      <dl class="row">
        ...
        <dd class="opt">
          <div id="gcategory">
            <select name="gc_id" class="class-select">
              <option value=""><?php echo $lang['nc_please_choose'];?></option>
              <?php if(!empty($output['category'])){ ?>
              <?php foreach($output['category']['result']['category'] as $k => $v){ ?>
              <option value="<?php echo $v['id'];?>" <?php if ($_GET['rec_gc_id'] == $v['id']):echo 'selected';endif;?>><?php echo $v['name'];?></option>
              <?php } ?>
              <?php } ?>
            </select>
          </div>
...
$(function(){
    ...
    $('#goods_search').on('click',function(){
    	...
    	if($("#goods_form").valid()){
    		...
    		<?php } ?>
    	    $('#show_recommend_goods_list').load('index.php?con=goods_recommend&fun=get_goods_list&gc_id='+gc_id+'&goods_name='+$('#goods_name').val());
    	}
    });
