...
//        }
        ...
        // 取得商品规格的输入值
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_price,goods_storage,goods_serial,goods_storage_alarm,goods_spec,goods_barcode,stone,carve,category_id,kind_id,texture_id');
        ...
        if (is_array($goods_array) && !empty($goods_array)) {
            ...
            foreach ( $goods_array as $k => $v ) {
                ...
                if (!empty($a)) {
                    ...
                    //$sp_value ['i_' . $id . '|stone'] = $v['stone'];
                    //$sp_value ['i_' . $id . '|carve'] = $v['carve'];
                }
            }
    ...
    public function goods_unshowOp() {
        ...
        if ($result['state']) {
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : 'index.php?con=store_goods_online&fun=goods_list', 'succ', '', 2);
        } else {
