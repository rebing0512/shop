...
final class Base
{
    ...
    private static function parse_conf(&$setting_config)
    {
        ...
        $setting['shop_version'] = '<span class="vol"><font class="b">shop</font><font class="o"></font></span>';
    ...
    private static function control()
    {
        ...
        if ($GLOBALS['setting_config']['enabled_subdomain'] == '1' && $_GET['con'] == 'index' && $_GET['fun'] == 'index') {
            $store_id = subdomain();
            if ($store_id > 0) $_GET['con'] = 'show_store';
        }
        $act_file = realpath(BASE_PATH . '/control/' . $_GET['con'] . '.php');
    ...
    private static function controladmin($admin)
    {
        ...
        $act_file = realpath(BASE_PATH . '/modules/' . $admin . '/control/' . $_GET['con'] . '.php');
    ...
    private static function start_session()
    {
        ...
        @ini_set('session.save_path', 'tcp://172.16.34.5:6379');

        //默认以文件形式存储session信息
        // @cevin 2017年8月31日 取消文件存储
