...
$(document).ready(function(){
    $("[nctype='article_attitude']").click(function() {
        ...
        $.getJSON("<?php echo urlShop('attitude','article_attitude');?>", { id: article_id, catid: catid,article_attitude: article_attitude }, function(json){
            if(json.result == "true") {
...
<ul id="cms_attitude_flag">
  ...
  <li nctype="article_attitude" attitude="<?php echo $i;?>" title="<?php echo $output['article_attitude_list'][$i];?>">
      <div class="article-attitude-percent-box"> <em nctype="article_attitude_count"><?php echo $output['data']['attitude_'.$i];?></em>
        <p class="article-attitude-percent-<?php echo $i;?>" nctype="article_attitude_percent" ></p>
      </div>
      <img src="<?php echo SHOP_TEMPLATES_URL;?>/images/attitude/<?php echo $i;?>.png" alt="" /> <span><?php echo $output['article_attitude_list'][$i];?></span>
  </li>
  <?php } ?>
</ul>
<div class="clear"></div>
