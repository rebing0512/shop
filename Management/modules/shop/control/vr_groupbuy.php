...
class vr_groupbuyControl extends SystemControl
{
    public function __construct()
    {
        ...
        if (C('groupbuy_allow') != 1) {
            showMessage('虚拟特卖功能尚未开启', 'index.php?con=setting', 'html', 'error');
        }
    }
    ...
    public function class_addOp()
    {
        if (chksubmit()) { //添加虚拟特卖分类
            ...
            if ($res) {
                ...
                $this->log('添加虚拟特卖分类[ID:'.$res.']', 1);

                $url = array(
                    array(
                        'url'=>'index.php?con=vr_groupbuy&fun=class_add&parent_class_id='.$params['parent_class_id'],
                        'msg'=>'继续添加',
                    ),
                    array(
                        'url'=>'index.php?con=vr_groupbuy&fun=class_list',
                        'msg'=>'返回列表',
                    )
                );
                showMessage('添加成功', $url);
            } else {
                showMessage('添加失败', 'index.php?con=vr_groupbuy&fun=class_list', '', 'error');
            }
        }
    ...
    public function class_editOp()
    {
        if (chksubmit()) {
            ...
            if ($res) {
                ...
                $this->log('编辑虚拟特卖分类[ID:'.intval($_POST['class_id']).']', 1);
                showMessage('编辑成功', 'index.php?con=vr_groupbuy&fun=class_list', '', 'succ');
            } else {
                showMessage('编辑失败', 'index.php?con=vr_groupbuy&fun=class_list', '', 'error');
            }
        }
    ...
    public function class_delOp()
    {
        ...
        $this->log('删除虚拟特卖分类[ID:'.$_POST['class_id'].']', 1);
        showMessage('删除成功', 'index.php?con=vr_groupbuy&fun=class_list', '', 'succ');
    }
...
<span class="btn">
  ...
  <ul>
    <li><a href="index.php?con=vr_groupbuy&fun=area_edit&area_id={$val['area_id']}">编辑区域</a></li>
    <li><a  href="index.php?con=vr_groupbuy&fun=area_view&parent_area_id={$val['area_id']}">下级区域</a></li>
  </ul>
</span>
EOB;
    ...
    public function area_addOp()
    {
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($res) {
                ...
                $this->log('添加虚拟特卖区域[ID:'.$res.']',1);
                showMessage('添加成功','index.php?con=vr_groupbuy&fun=area_list','','succ');
            } else {
                showMessage('添加失败','index.php?con=vr_groupbuy&fun=area_list','','error');
            }
        }
    ...
    public function area_editOp()
    {
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($res) {
                ...
                $this->log('编辑虚拟特卖区域[ID:'.intval($_POST['area_id']).']', 1);
                showMessage('编辑成功', 'index.php?con=vr_groupbuy&fun=area_list', '', 'succ');
            } else {
                showMessage('编辑失败', 'index.php?con=vr_groupbuy&fun=area_list', '', 'error');
            }
        }
    ...
    public function area_dropOp()
    {
        ...
        if ($res) {
            ...
            $this->log('删除虚拟特卖区域[ID:'.intval($_POST['area_id']).']',1);
            showMessage('删除成功','index.php?con=vr_groupbuy&fun=area_list','','succ');
        } else {
            showMessage('删除失败','index.php?con=vr_groupbuy&fun=area_list','','error');
        }
    }
