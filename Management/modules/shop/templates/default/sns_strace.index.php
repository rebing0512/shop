...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=sns_strace&fun=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?con=sns_strace&fun=scomm_list';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?con=sns_strace&fun=strace_del', {id:id}, function(data){
            if (data.state) {
