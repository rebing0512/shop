...
class promotion_bookControl extends SystemControl{
    private $links = array(
        array('url'=>'con=promotion_book&fun=index','text'=>'定金预售'),
        array('url'=>'con=promotion_book&fun=presell_goods_list', 'text'=>'全款预售'),
        array('url'=>'con=promotion_book&fun=book_quota_list','text'=>'套餐列表'),
        array('url'=>'con=promotion_book&fun=book_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?con=promotion_book&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
