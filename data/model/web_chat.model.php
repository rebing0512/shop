...
class web_chatModel extends Model{
    ...
    public function addMsg($msg) {
        ...
        if ($m_id > 0) {
            ...
            $goods_id = 0;
            $goods_info = array();
            $pattern = '#'.SHOP_SITE_URL.'/index.php\?con=goods&amp;fun=index&amp;goods_id=(\d+)$#';
            preg_match($pattern, $t_msg, $matches);
            if (intval($matches[1]) < 1) {//伪静态
                $pattern = '#'.SHOP_SITE_URL.'/goods-(\d+)\.html$#';
                preg_match($pattern, $t_msg, $matches);
            }
    ...
    public function getRecentFromList($condition = array(),$limit = 5,$member_list = array()) {
        ...
        if(!empty($list) && is_array($list)) {
            foreach($list as $k => $v) {
                ...
                $member['recent'] = 1;
                $member['time'] = date("Y-m-d H:i:s",$v['addtime']);
                $member['t_msg'] = !empty($v['t_msg']) ? $v['t_msg']:'';
                if(empty($member_list[$u_id])) {
                    $member_list[$u_id] = $member;
                } else {
                    $member_list[$u_id]['recent'] = 1;
                    $member_list[$u_id]['t_msg'] = !empty($v['t_msg']) ? $v['t_msg']:'';
                    $member_list[$u_id]['time'] = date("Y-m-d H:i:s",$v['addtime']);
                }
            }
        }
    ...
    public function getMemberFromList($condition = array(), $page = '', $limit = '') {
        ...
        if ($msg['count'] > 0) {
            $count = intval($msg['count']);
            if (C('dbdriver') == 'oracle') {
                $field = 'f_id,max(f_name) as f_name,max(add_time) as addtime,t_msg';
            } else {
                $field = 'f_id,f_name,max(add_time) as addtime,t_msg';
            }
    ...
    public function delChatMsg($condition){
        return (int) $this->table('chat_msg')->where($condition)->delete();
    }
