...
var zomslide_adv_list = new Array();//焦点大图广告数据
...
<?php if(!empty($output['zomslide_adv_list']) && is_array($output['zomslide_adv_list'])){ ?>
<?php foreach ($output['zomslide_adv_list'] as $key => $val) { ?>
adv_info = new Array();
ap_id = "<?php echo $val['ap_id'];?>";
adv_info['ap_id'] = ap_id;
adv_info['ap_name'] = "<?php echo $val['ap_name'];?>";
adv_info['ap_img'] = "<?php echo $val['default_content'];?>";
zomslide_adv_list[ap_id] = adv_info;
screen_adv_append += '<option value="'+ap_id+'">'+adv_info['ap_name']+'</option>';
<?php } ?>
<?php } ?>
...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        <li><a href="index.php?con=web_config&fun=web_config"><?php echo '板块区';?></a></li>
        <li><a href="JavaScript:void(0);" class="current"><?php echo '焦点区';?></a></li>
        <li><a href="index.php?con=web_config&fun=sale_edit"><?php echo '促销区';?></a></li>
      </ul>
    </div>
  </div>
  <div class="explanation" id="explanation">
    ...
    <ul>
      ...
      <li><?php echo '弹窗焦点大图手机端为了加载更快只显示前三张广告图';?></li>
    ...
    <div class="title">
      ...
      <ul class="tab-base nc-row">
        <li><a href="JavaScript:void(0);" class="current" form="upload_screen_form"><?php echo '全屏(背景)焦点大图';?></a></li>
         <li><a href="JavaScript:void(0);"  form="upload_zomslide_form"><?php echo '弹窗焦点大图';?></a></li>
        <li><a href="JavaScript:void(0);" form="upload_focus_form"><?php echo '三张联动焦点组图';?></a></li>
      </ul>
    </div>
    <form id="upload_screen_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?con=web_config&fun=screen_pic" target="upload_pic">
      ...
      <div class="ncap-form-default">
        <div class="bot"><a href="JavaScript:void(0);" onclick="$('#upload_screen_form').submit();" class="ncap-btn-big ncap-btn-green"><?php echo $lang['web_config_save'];?></a> <a href="index.php?con=web_config&fun=html_update&web_id=<?php echo $output['code_screen_list']['web_id'];?>" class="ncap-btn-big ncap-btn-green"><?php echo $lang['web_config_web_html'];?></a> <span class="web-save-succ" style="display:none;"><?php echo $lang['nc_common_save_succ'];?></span> </div>
      </div>
    </form>


     <form id="upload_zomslide_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?con=web_config&fun=zomslide_pic" target="upload_pic" style="display:none;">
      <input type="hidden" name="form_submit" value="ok" />
      <input type="hidden" name="web_id" value="<?php echo $output['code_zomslide_list']['web_id'];?>">
      <input type="hidden" name="code_id" value="<?php echo $output['code_zomslide_list']['code_id'];?>">
      <input type="hidden" name="key" value="">
      <div class="ncap-form-default">
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="site_status1" class="cb-enable <?php if($output['code_zomslide_list']['site_status'] == '1'){ ?>selected<?php } ?>" ><?php echo $lang['open'];?></label>
            <label for="site_status0" class="cb-disable <?php if($output['code_zomslide_list']['site_status'] == '0'){ ?>selected<?php } ?>" ><?php echo $lang['close'];?></label>
            <input id="site_status1" name="site_status" <?php if($output['code_zomslide_list']['site_status'] == '1'){ ?>checked="checked"<?php } ?>  value="1" type="radio">
            <input id="site_status0" name="site_status" <?php if($output['code_zomslide_list']['site_status'] == '0'){ ?>checked="checked"<?php } ?> value="0" type="radio">
          </div>
          <p class="notic"><?php echo $lang['site_state_notice'];?></p>
        </dd>
      </dl>
        <dl class="row">
          <dt class="tit">弹窗焦点大图预览</dt>
          <dd class="opt">
            <div class="full-screen-slides">
              <ul>
                <?php if (is_array($output['code_zomslide_list']['code_info']) && !empty($output['code_zomslide_list']['code_info'])) { ?>
    
                <?php foreach ($output['code_zomslide_list']['code_info'] as $keys => $vals) { ?>
                <?php if (is_array($vals) && $vals['ap_id'] > 0) { ?>
                <li ap="1" zomslide_id="<?php echo $vals['pic_id'];?>" title="可上下拖拽更改显示顺序">
                <div class="title"><h4>广告调用</h4><a class="ncap-btn-mini del" href="JavaScript:del_zomslide(<?php echo $vals['pic_id'];?>);" title="<?php echo $lang['nc_del'];?>"><i class="fa fa-trash"></i>删除</a></div>
                  <div class="focus-thumb" onclick="select_zomslide(<?php echo $vals['pic_id'];?>);" style="background-color:<?php echo $vals['color'];?>;" title="点击编辑选中区域内容"> <img title="<?php echo $vals['pic_name'];?>" src="<?php echo UPLOAD_SITE_URL.'/'.$vals['pic_img'];?>"/></div>
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][pic_id]" value="<?php echo $vals['pic_id'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][ap_id]" value="<?php echo $vals['ap_id'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][pic_name]" value="<?php echo $vals['pic_name'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][color]" value="<?php echo $vals['color'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][pic_img]" value="<?php echo $vals['pic_img'];?>" type="hidden">
                </li>
                <?php }else { ?>
                <li ap="0" zomslide_id="<?php echo $vals['pic_id'];?>" title="可上下拖拽更改显示顺序"><div class="title"><h4>图片调用</h4><a class="ncap-btn-mini del" href="JavaScript:del_zomslide(<?php echo $vals['pic_id'];?>);"><i class="fa fa-trash"></i>删除</a></div>
                  <div class="focus-thumb" onclick="select_zomslide(<?php echo $vals['pic_id'];?>);" style="background-color:<?php echo $vals['color'];?>;" title="点击编辑选中区域内容"> <img title="<?php echo $vals['pic_name'];?>" src="<?php echo UPLOAD_SITE_URL.'/'.$vals['pic_img'];?>"/></div>
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][pic_id]" value="<?php echo $vals['pic_id'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][pic_name]" value="<?php echo $vals['pic_name'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][pic_url]" value="<?php echo $vals['pic_url'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][mob_pic_url]" value="<?php echo $vals['mob_pic_url'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][color]" value="<?php echo $vals['color'];?>" type="hidden">
                  <input name="zomslide_list[<?php echo $vals['pic_id'];?>][pic_img]" value="<?php echo $vals['pic_img'];?>" type="hidden">
                </li>
                <?php } ?>
            ...
            <p class="notic"><?php echo '小提示：单击图片选中修改，拖动可以排序，添加最多不超过5个，保存后生效。';?></p>
            <div class="mt20"><a class="ncap-btn" href="JavaScript:add_zomslide('pic');"><?php echo '图片调用';?></a>
              <?php if(!empty($output['screen_adv_list']) && is_array($output['screen_adv_list'])){ ?>
              <a class="ncap-btn" href="JavaScript:add_zomslide('adv');"><?php echo '广告调用';?></a>
              <?php } ?>
            </div>
          </dd>
        </dl>
      </div>
      <div id="ap_zomslide" class="ncap-form-default" style="display:none; overflow: visible;">
        <div class="title">
          <h3>新增/选中区域内容设置详情</h3>
        </div>
        <dl class="row">
          ...
          <dd class="opt">
            <input type="hidden" name="ap_pic_id" value="">
            <select id="ap_id_screen" name="ap_id_screen" class=" w200" onchange="select_ap_zomslide();">
            </select>
            <span class="err"></span>
            <p class="notic">调用的数据是宽度为800像素，高度为400像素的图片类广告位。</p>
          </dd>
        </dl>
        <dl class="row" style="z-index: 3;">
          <dt class="tit">
          ...
          <dd class="opt">
            <input id="ap_color" name="ap_color" value="" class="" type="text">
            <span class="err"></span>
            <p class="notic">为确保显示效果美观，可设置首页弹窗焦点大图区域整体背景填充色用于弥补图片在不同分辨率下显示区域超出图片时的问题，可根据您焦点图片的基础底色作为参照进行颜色设置。</p>
          </dd>
        </dl>
      </div>
      <div id="upload_zomslide" class="ncap-form-default" style="display:none; overflow: visible;">
        <div class="title">
          <h3>新增/选中区域内容设置详情</h3>
        </div>
        <dl class="row">
          <dt class="tit"><?php echo '文字标题';?></dt>
          <dd class="opt">
            <input type="hidden" name="zomslide_id" value="">
            <input class="txt" type="text" name="zomslide_pic[pic_name]" value="">
            <span class="err"></span>
            <p class="notic">图片标题文字将作为图片Alt形式显示。</p>
          </dd>
        </dl>
        <dl class="row">
          <dt class="tit">
            <label><?php echo 'pc端url地址';?></label>
          </dt>
          <dd class="opt">
            <input name="zomslide_pic[pic_url]" value="" class="input-txt" type="text">
            <span class="err"></span>
            <p class="vatop tips">输入图片要跳转的URL地址，正确格式应以"http://"开头，点击后将以"_blank"形式另打开页面。</p>
          </dd>
        </dl>
        <dl class="row">
          <dt class="tit">
            <label><?php echo '手机端url地址'?></label>
          </dt>
          <dd class="opt">
            <input name="zomslide_pic[mob_pic_url]" value="" class="input-txt" type="text">
            <span class="err"></span>
            <p class="vatop tips">输入图片要跳转的URL地址，正确格式应以"http://"开头，点击后将以"_blank"形式另打开页面。</p>
          </dd>
        </dl>
        <dl class="row">
          ...
          <dd class="opt">
            ...
            <p class="notic">为确保显示效果正确，请选择800*400的清晰图片作为弹窗焦点大图。</p>
          </dd>
        </dl>
     <!--    <dl class="row" style="z-index: 3;">
          <dt class="tit">
          ...
          <dd class="opt">
            <input id="zomslide_color" name="zomslide_pic[color]" value="" class="" type="text">
            <span class="err"></span>
            <p class="notic">为确保显示效果美观，可设置首页弹窗焦点大图区域整体背景填充色用于弥补图片在不同分辨率下显示区域超出图片时的问题，可根据您焦点图片的基础底色作为参照进行颜色设置。</p>
          </dd>
        </dl> -->
      ...
      <div class="ncap-form-default">
        <div class="bot"><a href="JavaScript:void(0);" onclick="$('#upload_zomslide_form').submit();" class="ncap-btn-big ncap-btn-green"><?php echo $lang['web_config_save'];?></a> <a href="index.php?con=web_config&fun=html_update&web_id=<?php echo $output['code_zomslide_list']['web_id'];?>" class="ncap-btn-big ncap-btn-green"><?php echo $lang['web_config_web_html'];?></a> <span class="web-save-succ" style="display:none;"><?php echo $lang['nc_common_save_succ'];?></span> </div>
      </div>
    </form>


    <form id="upload_focus_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?con=web_config&fun=focus_pic" target="upload_pic" style="display:none;">
      <input type="hidden" name="form_submit" value="ok" />
      ...
      <div class="ncap-form-default">
        <div class="bot"> <a href="JavaScript:void(0);" onclick="$('#upload_focus_form').submit();" class="ncap-btn-big ncap-btn-green"><?php echo $lang['web_config_save'];?></a> <a href="index.php?con=web_config&fun=html_update&web_id=<?php echo $output['code_screen_list']['web_id'];?>" class="ncap-btn-big ncap-btn-green"><?php echo $lang['web_config_web_html'];?></a> <span class="web-save-succ" style="display:none;"><?php echo $lang['nc_common_save_succ'];?></span> </div>
      </div>
    </form>
  </div>
</div>
