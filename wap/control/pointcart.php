...
class pointcartControl extends BasePointShopControl{
    public function __construct() {
        ...
        if (C('pointprod_isuse') != 1){
             ...
             echo json_encode(array('done'=>false,'msg'=>'系统没有开启积分兑换功能!','url'=>'index.php')); die;
        }
        //验证是否登录
        if ($_SESSION['is_login'] != '1'){
 
            echo json_encode(array('done'=>false,'msg'=>'对不起,你还没登录!','url'=>urlMobile('login'))); die;
    ...
    public function addOp() {
        ...
        if($pgid <= 0 || $quantity <= 0) {
            echo json_encode(array('done'=>false,'msg'=>'兑换失败')); die;
        }
        ...
        if (!$data['state']){
            switch ($data['error']){
                case 'ParameterError':
                    echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>'index.php?con=pointprod&fun=plist')); die;
                    break;
                default:
    ...
    public function step1Op(){
        ...
        //获取符合条件的兑换礼品和总积分
        $data = Model('pointcart')->getCartGoodsList($_SESSION['member_id'],$plist);
        if (!$data['state']){
          
            showMessage($data['msg'],urlMobile('pointcart'),'html','error');
        }

        Tpl::output('pointprod_arr',$data['data']);
        $web_seo = C('site_name')."-确认收货人资料";
        Tpl::output('web_seo',$web_seo); 
        // //实例化收货地址模型（不显示自提点地址）
        $address_list = Model('address')->getAddressList(array('member_id'=>$_SESSION['member_id'],'dlyp_id'=>0), 'is_default desc,address_id desc');
    
        Tpl::output('address_list',$address_list[0]);
    ...
    public function step2Op() {
        ...
        $url = urlMobile('pointcart','step3',array('order_id'=>$order_id));
        @header("Location:$url");
    }
    /**
     * 流程第三步
