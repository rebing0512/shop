...
class stat_tradeControl extends SystemControl{
    private $links = array(
        array('url'=>'con=stat_trade&fun=income','lang'=>'stat_sale_income'),
        array('url'=>'con=stat_trade&fun=predeposit','lang'=>'stat_predeposit'),
        array('url'=>'con=stat_trade&fun=sale','lang'=>'stat_sale')
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['fun'],array('sale_trend','get_sale_xml','get_plat_sale'))){
    ...
    public function get_income_xmlOp(){
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                $out_array = array();
                $out_array['operation'] = '';
                //$out_array['operation'] = '<a class="btn green" href="index.php?con=stat_trade&fun=sale&search_type=month&searchmonth_month='.$_GET['search_month'].
