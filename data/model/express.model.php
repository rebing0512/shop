...
class expressModel extends Model {
    ...
    * @param unknown $key    快递申请的API ID ,没有的，请去快递申请。
    * @return multitype:
    */
    function get_express($e_code, $shipping_code) {
  
        $key = C('site_kuaidi');
        $url ='http://api.kuaidi100.com/api?id='.$key.'&com='.$e_code.'&nu='.$shipping_code.'&show=0&muti=1&order=asc';
       ...
       if (function_exists('curl_init') == 1){
        ...
        if ($content['status'] != true || !is_array($content['data'])) {
            return array();
        }
