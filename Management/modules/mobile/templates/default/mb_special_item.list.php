...
<div class="page"> 
  <!-- 页面导航 -->
  <div class="fixed-bar">
    <div class="item-title"><?php if ($output['special_id'] > 0) {?><a class="back" href="index.php?con=mb_special&fun=special_list" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a><?php }?>
      <div class="subject">
...
"></i><?php echo $value['usable_text'];?></a> <a nctype="btn_edit_item" data-item-id="<?php echo $value['item_id'];?>" href="javascript:;" goods_type="<?php echo $value['goods_type'];?>"><i class="fa fa-pencil-square-o"></i>编辑</a> <a nctype="btn_del_item" data-item-id="<?php echo $value['item_id'];?>" href="javascript:;" ><i class="fa fa-trash-o
"></i>删除</a></div>
          </td>
        </div>
        <?php } ?>
    ...
    <div class="module-list">
      ...
      <div class="module_<?php echo $key;?>"> <span><?php echo $value['desc'];?></span> <a nctype="btn_add_item" class="add" href="javascript:;" data-module-type="<?php echo $value['name'];?>" data-goods-type="<?php echo $value['type'];?>">添加</a> </div>
      <?php } ?>
      <?php } ?>
    </div>
  </div>
    ...
    $(document).ready(function(){
        ...
        $('[nctype="btn_add_item"]').on('click', function() {
            var data = {
                ...
                goods_type: $(this).attr('data-goods-type'),
        ...
        $('#item_list').on('click', '[nctype="btn_edit_item"]', function() {
            ...
            go(url_item_edit + '&item_id=' + item_id+ '&goods_type=' + goods_type);
        });
        ...
        var update_item_sort = function() {
            ...
            $.post("index.php?con=mb_special&fun=update_item_sort", {special_id: special_id, item_id_string: item_id_string}, function(data) {
                if(typeof data.error != 'undefined') {
        ...
        $('#item_list').on('click', '[nctype="btn_usable"]', function() {
            ...
            $.post("index.php?con=mb_special&fun=update_item_usable", {item_id: item_id, usable: usable, special_id: special_id}, function(data) {
                if(typeof data.error != 'undefined') {
                    showError(data.message);
                }
            }, 'json');
