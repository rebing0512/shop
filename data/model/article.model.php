...
class articleModel extends Model {
     ...
     * @return array 数组结构的返回结果
     */
    public function getArticleCommentList($condition,$page=''){
        ...
        $param['where'] = $condition_str;
        $param['limit'] = $condition['limit'];
        $param['order'] = (empty($condition['order'])?'s_comment_id desc,s_comment_time desc':$condition['order']);
        $result = Db::select($param,$page);
        return $result;
    }
    ...
    private function _condition($condition){
        ...
        if($condition['s_conmmet_article_id'] !=''){
            $condition_str .=" s_conmmet_article_id = ".$condition['s_conmmet_article_id'];
        }
    ...
    public function delpl($id){
        if (intval($id) > 0){
            $where = "s_comment_id = '". intval($id) ."'";
            $result = Db::delete('article_comment',$where);
            return $result;
        }else {
            return false;
        }
