...
class Route {
    ...
    private function _parseRuleRewrite() {
        ...
        if (!empty($path_info) && $path_info != 'index.php' && strpos($path_info, $this->_rewrite_extname)){
            ...
            //con,fun强制赋值
            $_GET['con'] = isset($split_array[0]) ? $split_array[0] : 'index';
    ...
    private function _shopPathInfo($path_info) {
        $reg_match_from = array(
            ...
            '/^glist-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)$/',
            '/^gsoon-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)$/',
            '/^ghistory-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)$/',

            '/^vr_glist-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)$/',
            '/^vr_gsoon-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)$/',
            '/^vr_ghistory-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)-(\d+)$/',
