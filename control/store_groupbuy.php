...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('groupbuy_allow')) !== 1){
            showMessage(Language::get('groupbuy_unavailable'),'index.php?con=seller_center','','error');
        }
    }
    /**
     * 默认显示特卖列表
    ...
    public function groupbuy_quota_add_saveOp() {
        ...
        //记录店铺费用
        $this->recordStoreCost($current_price * $groupbuy_quota_quantity, '购买特卖');

        $this->recordSellerLog('购买'.$groupbuy_quota_quantity.'份特卖套餐，单价'.$current_price.L('nc_yuan'));
    ...
    public function groupbuy_saveOp() {
        ...
        if ($_GET['vr']) {
            if ($param['upper_limit'] > 0 && $goods_info['virtual_limit'] > 0
                ...
                showDialog(sprintf('虚拟特卖活动的限购数量(%d)不能大于虚拟商品本身的限购数量(%d)',
                    $param['upper_limit'], $goods_info['virtual_limit']
                    ), 'index.php?con=store_groupbuy');
            }
        ...
        if($result) {
            ...
            $this->recordSellerLog('发布特卖活动，特卖名称：'.$param['groupbuy_name'].'，商品名称：'.$param['goods_name']);
            showDialog(Language::get('groupbuy_add_success'),'index.php?con=store_groupbuy','succ');
        }else {
            showDialog(Language::get('groupbuy_add_fail'),'index.php?con=store_groupbuy');
        }
    }
    ...
    private function profile_menu($menu_key='') {
        ...
        switch ($menu_key){
        case 'groupbuy_add':
            $menu_array[] = array('menu_key'=>'groupbuy_add','menu_name'=>L('nc_member_path_new_group'),'menu_url'=>'index.php?con=store_groupbuy&fun=groupbuy_add');
            break;
        case 'groupbuy_add_vr':
            $menu_array[] = array('menu_key'=>'groupbuy_add_vr','menu_name'=>'新增虚拟特卖','menu_url'=>'index.php?con=store_groupbuy&fun=groupbuy_add_vr');
            break;
        case 'groupbuy_quota_add':
            ...
            break;
        case 'groupbuy_edit':
            $menu_array[] = array('menu_key'=>'groupbuy_edit','menu_name'=>L('nc_member_path_edit_group'),'menu_url'=>'index.php?con=store_groupbuy');
            break;
        case 'cancel':
