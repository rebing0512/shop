...
class goodsLogic {
    ...
    public function saveGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc=1) {
        ...
        if (!$category)
            return callback(false,'分类选择错误');

        $model_goods = Model('goods');

        // 根据参数初始化通用商品数据
        $common_array = $this->_initCommonGoodsByParam($param, $store_id, $store_name, $store_state,$category);
...
//        }
    ...
    public function updateGoods($param, $store_id, $store_name, $store_state, $seller_id, $seller_name, $bind_all_gc=1) {
        $model_goods = Model('goods');
        $common_id = intval($param['commonid']);
        if ($common_id <= 0) {
            return callback(false, 'COMMONID不存在');
        }
        // 验证参数
        $error = $this->_validParam($param);
        if ($error != '') {
...
//        }
     ...
     * 根据参数初始化通用商品数据
     */
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state, array $_cate_=[]) {
        // 分类信息
        $goods_class = Model('goods_class')->getGoodsClassLineForTag(intval($param['cate_id']));

        $common_array = array();
        $common_array['goods_name']         = $param['g_name'];
        $common_array['stone']              = $param['stone'];
        $common_array['carve']              = $param['carve'];
        ...
        $common_array['gc_name']            = $_cate_['category'];
        ...
        $common_array['_size'] = $param['g_size'];
        $common_array['category_id'] = $_POST['category_id'];
        $common_array['texture_id'] = $_POST['texture_id'];
        $common_array['kind_id'] = $_POST['attribute_id'];
        $common_array['texture'] = $_POST['texture'];
        $common_array['attribute'] = $_POST['attribute'];


        file_put_contents(BASE_ROOT_PATH.DS.'a.txt',$common_array['goods_stcids']);
        return $common_array;
    }
    /**
     * 序列化保存手机端商品描述数据
    ...
    private function _addGoods($param, $common_id, $common_array) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                $goods['stone']             = $value['stone'];
                $goods['carve']             = $value['carve'];
            ...
            $goods['color_id']          = 0;
            $goods['stone']             = $common_array['stone'];
            $goods['carve']             = $common_array['carve'];
            $goods_id = $model_goods->addGoods($goods);
            $model_type->addGoodsType($goods_id, $common_id, array('cate_id' => $param['cate_id'], 'type_id' => $param['type_id'], 'attr' => $param['attr']));
    ...
    private function _editGoods($param, $common_id, $common_array, $store_id,array $_cate_=[]) {
        $goodsid_array = array();
        $colorid_array = array();
        ...
        if (is_array($param['spec'])) {
            ...
            if (!empty($goods_info)) {
                ...
                $goods['stone']             = $common_array['stone'];
                $goods['carve']             = $common_array['carve'];
                $goods['category_id']       = $common_array['category_id'];
                $goods['kind_id']           = $_POST['attribute_id'];
                $goods['texture_id']        = $common_array['texture_id'];
                $godos['gc_name']           = $_cate_['category'];
                $goods['color_id']          = 0;
                if ($common_array['is_virtual'] == 1) {
                    $goods['have_gift']    = 0;
                    Model('goods_gift')->delGoodsGift(array('goods_id' => $goods_id));
     ...
     * 根据通用商品数据初始化商品数据
     */
    private function _initGoodsByCommonGoods($common_id, $common_array,array $_cate_=[]) {
        $goods = array();
        ...
        $goods['category_id'] = $common_array['category_id'];
        $goods['texture_id'] = $common_array['texture_id'];
        $goods['kind_id'] = $common_array['kind_id'];
        $goods['texture'] = $common_array['texture'];
        $goods['attribute'] = $common_array['attribute'];
    ...
    public function uploadGoodsImage($image_name, $store_id, $album_limit)
    {
        ...
        if (!$class_info)
        {
            ...
            $id = Model('album_class')->insert($class_info);
            $class_info['aclass_id'] = $id;
        }
        // 上传图片
