  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        ...
        <li><a href="index.php?con=circle_manage&fun=circle_verify"><?php echo $lang['circle_wait_verify'];?></a></li>
      </ul>
    </div>
  </div>
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?con=circle_manage&fun=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?con=circle_manage&fun=circle_add';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?con=circle_manage&fun=circle_del', {id:id}, function(data){
            if (data.state) {
